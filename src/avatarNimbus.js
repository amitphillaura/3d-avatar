...
// Fluffy storm-cloud being — a lumpy cloud body with falling rain and the occasional
// lightning flash. The humanoid rig is hidden; the cloud cluster IS the body.
const NIMBUS = {
  cloud: 0xeef2f7,
  cloudShade: 0xc3ccd8,
  rain: 0x7fc8ff,
...
const BONE_STYLE = {
  collar: { radius: 0.055, color: NIMBUS.cloud },
  hip: { radius: 0.06, color: NIMBUS.cloud },
  arm: { radius: 0.045, color: NIMBUS.cloud },
  leg: { radius: 0.052, color: NIMBUS.cloud },
  side: { radius: 0.04, color: NIMBUS.cloud }
};
...
export class MushyNimbus {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(NIMBUS.cloud),
      right: this.buildHandRig(NIMBUS.cloud)
    };
    this.resize();
  ...
  createRig() {
    ...
    // Humanoid end-caps still exist (the base loop drives them) but stay hidden every
    // frame — the cloud cluster is the body.
    const capMaterial = makeMaterial(NIMBUS.cloud, 0.85);
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.SphereGeometry(0.12, 12, 12), capMaterial),
      rightWrist: new THREE.Mesh(new THREE.SphereGeometry(0.12, 12, 12), capMaterial),
      leftAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.13, 12, 12), capMaterial),
      rightAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.13, 12, 12), capMaterial)
    };
    ...
    this.neck = new THREE.Mesh(
      ...
      makeMaterial(NIMBUS.cloud, 0.85)
    );
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeMaterial(style.color, 0.85);
    ...
    // Lumpy cloud torso: a base sphere plus a handful of overlapping white puffs.
    this.torso = new THREE.Mesh(
      new THREE.SphereGeometry(0.6, 24, 24),
      makeMaterial(NIMBUS.cloud, 0.92)
    );
    this.root.add(this.torso);
    const cloudLumps = [
      { x: -0.45, y: -0.05, z: 0.05, r: 0.42 },
      { x: 0.5, y: -0.02, z: -0.04, r: 0.46 },
      { x: -0.18, y: 0.28, z: 0.08, r: 0.36 },
      { x: 0.22, y: 0.3, z: -0.06, r: 0.34 },
      { x: 0.0, y: -0.22, z: 0.12, r: 0.4 },
      { x: 0.15, y: 0.02, z: 0.3, r: 0.32 }
    ];
    cloudLumps.forEach(({ x, y, z, r }) => {
    ...
    // Head: a smaller cloud puff with a sleepy/grumpy face.
    ...
    [
      { x: -0.22, y: 0.02, z: 0.0, r: 0.18 },
      { x: 0.22, y: 0.0, z: 0.02, r: 0.2 },
      { x: 0.0, y: 0.18, z: -0.02, r: 0.16 }
    ].forEach(({ x, y, z, r }) => {
      const lump = new THREE.Mesh(
    ...
    const eyeMaterial = new THREE.MeshBasicMaterial({ color: NIMBUS.eye });
    this.leftEye = new THREE.Mesh(new THREE.SphereGeometry(0.04, 12, 12), eyeMaterial);
    this.rightEye = new THREE.Mesh(new THREE.SphereGeometry(0.04, 12, 12), eyeMaterial);
    this.head.add(this.leftEye, this.rightEye);
    this.leftEye.position.set(-0.1, 0.02, 0.27);
    this.rightEye.position.set(0.1, 0.02, 0.27);
    // Small flat grumpy frown.
    const frown = new THREE.Mesh(
      new THREE.BoxGeometry(0.16, 0.025, 0.02),
      new THREE.MeshBasicMaterial({ color: NIMBUS.eye })
    );
    frown.position.set(0, -0.12, 0.27);
    this.head.add(frown);

    // Antenna — a tiny cloud puff on top (kept as `this.antenna` for the idle wiggle).
    ...
    const puffLump = new THREE.Mesh(
      new THREE.SphereGeometry(0.07, 12, 12),
      makeMaterial(NIMBUS.cloud, 0.92)
    );
    puffLump.position.set(0.08, 0.04, 0);
    this.antenna.add(puff, puffLump);
  ...
  // THE TECHNIQUE 1 — a pool of falling rain drops shed from the cloud bottom.
  buildRain() {
    this._rain = [];
    for (let i = 0; i < 22; i += 1) {
      const drop = new THREE.Mesh(
        new THREE.SphereGeometry(0.035, 6, 6),
        new THREE.MeshBasicMaterial({ color: NIMBUS.rain, transparent: true, opacity: 0.8 })
      );
      drop.scale.set(1, 2.2, 1);
      drop.visible = false;
      this.root.add(drop);
      this._rain.push({ mesh: drop, speed: 1.4 + Math.random() * 1.6 });
    }
  }

  // THE TECHNIQUE 2 — a zigzag lightning bolt that flashes on occasionally.
  buildBolt() {
    this._bolt = new THREE.Group();
    const boltMaterial = new THREE.MeshBasicMaterial({ color: NIMBUS.bolt });
    const segs = [
      { x: 0.04, y: 0.0, z: 0, rot: 0.5, len: 0.34 },
      { x: -0.04, y: -0.26, z: 0, rot: -0.5, len: 0.32 },
      { x: 0.05, y: -0.5, z: 0, rot: 0.55, len: 0.3 }
    ];
    segs.forEach(({ x, y, z, rot, len }) => {
      const bar = new THREE.Mesh(new THREE.BoxGeometry(0.05, len, 0.05), boltMaterial);
      bar.position.set(x, y, z);
      bar.rotation.z = rot;
      this._bolt.add(bar);
    });
    this._bolt.visible = false;
    this.root.add(this._bolt);
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyNimbus tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyNimbus waiting for your body pose";
  ...
  // Hide the humanoid rig, then rain falls from the cloud bottom and lightning flashes.
  syncAttachedModel(delta) {
    ...
    this._rain.forEach((drop) => {
      const mesh = drop.mesh;
      if (!mesh.visible) {
        mesh.position.set(
          tp.x + (Math.random() - 0.5) * 1.4,
          tp.y - 0.4 - Math.random() * 1.0,
          tp.z + (Math.random() - 0.5) * 0.8
        );
        mesh.visible = true;
      }
      mesh.position.y -= drop.speed * delta;
      if (mesh.position.y < tp.y - 1.6) {
        mesh.position.set(
          tp.x + (Math.random() - 0.5) * 1.4,
          tp.y - 0.4,
          tp.z + (Math.random() - 0.5) * 0.8
    ...
    // Lightning: position under the cloud and flash rarely.
    this._bolt.position.set(tp.x, tp.y - 0.9, tp.z);
    this._bolt.visible = Math.sin(t * 0.7) > 0.985;
  }
