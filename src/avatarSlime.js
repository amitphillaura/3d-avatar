...
// Gooey jelly slime — glossy translucent green, with squash-and-stretch jiggle.
const SLIME = { body: 0x46d6a0, eye: 0x10241c, mouth: 0x0c3326 };

const BONE_STYLE = {
  collar: { radius: 0.08, color: SLIME.body },
  hip: { radius: 0.09, color: SLIME.body },
  arm: { radius: 0.075, color: SLIME.body },
  leg: { radius: 0.09, color: SLIME.body },
  side: { radius: 0.08, color: SLIME.body }
};
...
// Glossy translucent goo.
function makeSlimeMaterial(color, opacity = 0.85) {
  return new THREE.MeshStandardMaterial({
    color,
    roughness: 0.18,
    metalness: 0,
...
export class MushySlime {
  constructor(mount, metaElement, options = {}) {
    ...
    this.scene.background = new THREE.Color(0x081512);
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(SLIME.body),
      right: this.buildHandRig(SLIME.body)
    };
    this.resize();
  ...
  createRig() {
    ...
    const jointMaterial = makeSlimeMaterial(SLIME.body, 0.85);
    ...
    // Gooey blob hands and feet.
    const capMaterial = makeSlimeMaterial(SLIME.body, 0.85);
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.SphereGeometry(0.15, 16, 16), capMaterial),
      rightWrist: new THREE.Mesh(new THREE.SphereGeometry(0.15, 16, 16), capMaterial),
      leftAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.16, 16, 16), capMaterial),
      rightAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.16, 16, 16), capMaterial)
    };
    ...
    this.neck = new THREE.Mesh(
      ...
      makeSlimeMaterial(SLIME.body, 0.85)
    );
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeSlimeMaterial(style.color, 0.85);
    ...
    // Base torso/head exist for positioning but stay hidden (see syncAttachedModel) —
    // the jiggly blob is the real body.
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 24, 24), makeSlimeMaterial(SLIME.body, 0.85));
    this.root.add(this.torso);
    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.32, 24, 24), makeSlimeMaterial(SLIME.body, 0.85));
  ...
  buildBlob() {
    ...
    const bodyMaterial = makeSlimeMaterial(SLIME.body, 0.85);
    const body = new THREE.Mesh(new THREE.SphereGeometry(0.9, 28, 28), bodyMaterial);
    ...
    shine.position.set(-0.3, 0.42, 0.55);
    this.blob.add(shine);

    const eyeMaterial = new THREE.MeshBasicMaterial({ color: SLIME.eye });
    const eyeGeometry = new THREE.SphereGeometry(0.13, 16, 16);
    this.leftEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.leftEye.position.set(-0.24, 0.12, 0.76);
    this.rightEye.position.set(0.24, 0.12, 0.76);
    this.blob.add(this.leftEye, this.rightEye);
    [-1, 1].forEach((s) => {
      const glint = new THREE.Mesh(new THREE.SphereGeometry(0.04, 10, 10), new THREE.MeshBasicMaterial({ color: 0xffffff }));
      glint.position.set(s * 0.24 + 0.05, 0.17, 0.86);
      this.blob.add(glint);
    });

    const smile = new THREE.Mesh(
      new THREE.TorusGeometry(0.17, 0.03, 10, 24, Math.PI),
      new THREE.MeshBasicMaterial({ color: SLIME.mouth })
    );
    ...
    [-0.38, 0.02, 0.42].forEach((x, i) => {
      const drip = new THREE.Mesh(new THREE.SphereGeometry(0.11 - i * 0.012, 12, 12), bodyMaterial);
      drip.position.set(x, -0.84 - (i % 2) * 0.08, 0.08 * i);
      drip.scale.set(0.8, 1.35, 0.8);
      this.blob.add(drip);
    });

    // Bubble on top — kept as `this.antenna` for the inherited idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.85, 0);
    const bubble = new THREE.Mesh(new THREE.SphereGeometry(0.11, 14, 14), makeSlimeMaterial(SLIME.body, 0.7));
    bubble.position.y = 0.12;
    this.antenna.add(bubble);
    this.blob.add(this.antenna);
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushySlime tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushySlime waiting for your body pose";
  ...
  // Squash-and-stretch jiggle: a damped spring on the blob's height, kicked by how fast
  // the body is moving, plus a constant idle wobble. Volume is roughly preserved.
  syncAttachedModel(delta) {
    ...
    if (!this.blob || !this.torso) return;

    const pos = this.torso.position;
    this.blob.position.copy(pos);
    const dt = Math.max(delta || 1 / 60, 1 / 120);
    const speed = pos.distanceTo(this._prevPos) / dt;
    this._prevPos.copy(pos);

    this._wobbleVel += -this._wobble * 90 * dt;
    this._wobbleVel *= Math.max(0, 1 - 6 * dt);
    this._wobbleVel += Math.min(speed, 8) * 0.5 * dt;
    this._wobble = Math.max(-0.35, Math.min(0.35, this._wobble + this._wobbleVel * dt));

    const idle = Math.sin((performance.now() - this.startedAt) / 1000 * 3.2) * 0.04;
    const sy = 1 + this._wobble + idle;
