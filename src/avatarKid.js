...
// Cartoony "Cocomelon-kid" palette — chunky limbs + bright primary clothing.
const KID = {
  skin: 0xffcfae, // head, arms, hands
  shirt: 0xff6b6b, // torso + collar + sides
  pants: 0x4a6cf7, // hips + legs
  shoe: 0xfff1d6, // foot caps
  hair: 0x5a3a26, // hair tuft
  cheek: 0xff9fb0, // rosy cheeks
  eyeWhite: 0xffffff,
  pupil: 0x241712,
  nose: 0xffb38c,
  mouth: 0xb0364a
...
// Much thicker bones than Mushy so the kid reads chunky/inflated, with clothing
// colors: skin-tone bare arms, a bright shirt across the collar/sides, blue pants.
const BONE_STYLE = {
  collar: { radius: 0.1, color: KID.shirt },
  hip: { radius: 0.11, color: KID.pants },
  arm: { radius: 0.085, color: KID.skin },
  leg: { radius: 0.1, color: KID.pants },
  side: { radius: 0.09, color: KID.shirt }
};
...
export class MushyKid {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(KID.skin),
      right: this.buildHandRig(KID.skin)
    };
    this.resize();
  ...
  createRig() {
    // Chunky skin-tone joints so the inflated balloon limbs blend at the seams.
    const jointGeometry = new THREE.SphereGeometry(0.1, 20, 20);
    const jointMaterial = makeMaterial(KID.skin, 0.7);
    ...
    // Big mitten hands + chunky shoes so wrists read as hands and ankles as feet.
    const handGeometry = new THREE.SphereGeometry(0.17, 18, 18);
    const footGeometry = new THREE.SphereGeometry(0.16, 18, 18);
    const handMaterial = makeMaterial(KID.skin, 0.7);
    const footMaterial = makeMaterial(KID.shoe, 0.55);
    this.caps = {
    ...
    // Squash + lengthen the ankle caps so they read as shoes, not balls.
    this.caps.leftAnkle.scale.set(1.05, 0.72, 1.4);
    this.caps.rightAnkle.scale.set(1.05, 0.72, 1.4);
    Object.values(this.caps).forEach((cap) => {
      cap.visible = false;
      this.root.add(cap);
    });

    // Neck connects the torso/shoulders to the head — skin tone, nice and thick.
    ...
    // Rounder, deeper belly than Mushy — opaque bright shirt instead of the glass blob.
    this.torso = new THREE.Mesh(
      new THREE.SphereGeometry(0.82, 32, 32),
      makeMaterial(KID.shirt, 0.7)
    );
    this.torso.scale.set(0.98, 1.05, 0.62);
    this.root.add(this.torso);

    // Oversized cartoon head (Cocomelon-style big noggin).
    this.headRadius = 0.46;
    this.head = new THREE.Mesh(
      new THREE.SphereGeometry(this.headRadius, 32, 32),
      makeMaterial(KID.skin, 0.78)
    );
    this.root.add(this.head);
    this.buildFace();
  }

  // Procedural face on the head sphere: wide eyes (white + pupil + glint), a button
  // nose, rosy cheeks, a smile, and a hair tuft. Everything parents to `this.head`, so
  // it follows head position/rotation/scale exactly like the original Mushy eyes did.
  buildFace() {
    ...
    const pupilMat = new THREE.MeshBasicMaterial({ color: KID.pupil });
    const glintMat = new THREE.MeshBasicMaterial({ color: 0xffffff });

    const buildEye = (sign) => {
      const eye = new THREE.Group();
      eye.position.set(sign * 0.19, 0.08, R * 0.8);

      const white = new THREE.Mesh(new THREE.SphereGeometry(0.14, 18, 18), eyeWhiteMat);
      white.scale.set(1, 1.15, 0.7); // tall ovals = wide cartoon eyes
      eye.add(white);

      const pupil = new THREE.Mesh(new THREE.SphereGeometry(0.075, 14, 14), pupilMat);
      pupil.position.set(0, -0.01, 0.085);
      eye.add(pupil);

      const glint = new THREE.Mesh(new THREE.SphereGeometry(0.026, 10, 10), glintMat);
      glint.position.set(sign * 0.03, 0.045, 0.12);
      eye.add(glint);
    ...
    // Button nose.
    this.nose = new THREE.Mesh(new THREE.SphereGeometry(0.062, 16, 16), makeMaterial(KID.nose, 0.7));
    this.nose.position.set(0, -0.02, R * 0.99);
    this.nose.scale.set(1, 0.85, 0.8);
    this.head.add(this.nose);

    // Rosy cheeks — soft translucent blush so they melt into the skin.
    ...
    const buildCheek = (sign) => {
      const cheek = new THREE.Mesh(new THREE.SphereGeometry(0.085, 14, 14), cheekMat);
      cheek.position.set(sign * 0.27, -0.08, R * 0.78);
    ...
    // Smile: bottom half of a torus ring (rotated 180° about Z so it bows down into a ∪).
    this.mouth = new THREE.Mesh(
      new THREE.TorusGeometry(0.13, 0.028, 10, 28, Math.PI),
      makeMaterial(KID.mouth, 0.5)
    );
    this.mouth.position.set(0, -0.18, R * 0.85);
    this.mouth.rotation.z = Math.PI;
    this.head.add(this.mouth);

    // Hair tuft on the crown. Kept as `this.antenna` so the idle wiggle (antenna.rotation.z)
    // still sways it. A Group has .rotation just like a Mesh, so the existing code works.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, R * 0.92, 0.04);
    const hairMat = makeMaterial(KID.hair, 0.85);
    const lumps = [
      { r: 0.15, x: 0, y: 0.02, z: 0, sy: 1.35 },
      { r: 0.1, x: 0.14, y: -0.02, z: 0, sy: 1.0 },
      { r: 0.09, x: -0.13, y: -0.02, z: 0.02, sy: 1.0 }
    ];
    lumps.forEach(({ r, x, y, z, sy }) => {
      const lump = new THREE.Mesh(new THREE.SphereGeometry(r, 14, 14), hairMat);
      lump.position.set(x, y, z);
      lump.scale.set(1, sy, 1);
      this.antenna.add(lump);
    });
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyKid tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyKid waiting for your body pose";
  ...
  updateRig(delta) {
    ...
    if (bodyTracking) {
      ...
      if (hasCore) {
        ...
        this.torso.scale.set(shoulderWidth * 0.72, torsoHeight * 0.82, 0.52);
        ...
        this.head.scale.setScalar(Math.max(1.0, shoulderWidth * 0.6));
      }
    }
