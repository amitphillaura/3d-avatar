...
// A cute purple octopus. The humanoid rig is hidden every frame; the big round head IS
// the octopus body, with six waving tentacles (multiple follow-chains) hanging from it.
const OCTO = {
  body: 0x9b59b6,
  sucker: 0xd9a8e8,
  eye: 0xffffff,
...
const BONE_STYLE = {
  collar: { radius: 0.06, color: OCTO.body },
  hip: { radius: 0.06, color: OCTO.body },
  arm: { radius: 0.05, color: OCTO.body },
  leg: { radius: 0.05, color: OCTO.body },
  side: { radius: 0.05, color: OCTO.body }
};
...
export class MushyOcto {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(OCTO.body),
      right: this.buildHandRig(OCTO.body)
    };
    this.resize();
  ...
  createRig() {
    // The humanoid bones/joints/caps still exist (the base loop drives them) but stay
    // hidden every frame — the bulb head is the octopus body and the tentacles are the limbs.
    const jointGeometry = new THREE.SphereGeometry(0.06, 12, 12);
    const jointMaterial = makeMaterial(OCTO.body, 0.5);
    ...
    const capMaterial = makeMaterial(OCTO.body, 0.5);
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.SphereGeometry(0.1, 12, 12), capMaterial),
      rightWrist: new THREE.Mesh(new THREE.SphereGeometry(0.1, 12, 12), capMaterial),
      leftAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.1, 12, 12), capMaterial),
      rightAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.1, 12, 12), capMaterial)
    };
    ...
    this.neck = new THREE.Mesh(
      new THREE.CylinderGeometry(neckStyle.radius, neckStyle.radius, 1, 12),
      makeMaterial(OCTO.body, 0.5)
    );
    ...
    // Hidden placeholder torso (kept for the HARD CONTRACT; the head bulb is the body).
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.4, 12, 12), makeMaterial(OCTO.body, 0.5));
    this.torso.visible = false;
    this.root.add(this.torso);

    // The big round head IS the octopus body — a purple bulb, slightly tall.
    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.42, 32, 32), makeMaterial(OCTO.body, 0.5));
    this.head.scale.set(1, 1.12, 1);
  ...
  buildOctoFace() {
    const R = 0.42;
    // Huge white eyes with dark pupils.
    const eyeMaterial = makeBasic(OCTO.eye);
    const pupilMaterial = makeBasic(OCTO.pupil);
    const eyes = [];
    [-1, 1].forEach((s) => {
      const eye = new THREE.Mesh(new THREE.SphereGeometry(0.13, 16, 16), eyeMaterial);
      eye.position.set(s * 0.16, 0.08, R * 0.78);
      this.head.add(eye);
      eyes.push(eye);
      const pupil = new THREE.Mesh(new THREE.SphereGeometry(0.06, 12, 12), pupilMaterial);
      pupil.position.set(s * 0.18, 0.06, R * 0.9);
      this.head.add(pupil);
    });
    this.leftEye = eyes[0];
    ...
    // Pink cheeks.
    const cheekMaterial = makeBasic(OCTO.cheek);
    [-1, 1].forEach((s) => {
      const cheek = new THREE.Mesh(new THREE.SphereGeometry(0.055, 12, 12), cheekMaterial);
      cheek.position.set(s * 0.24, -0.08, R * 0.7);
      this.head.add(cheek);
    });

    // Small mouth.
    const mouth = new THREE.Mesh(
      new THREE.TorusGeometry(0.05, 0.016, 8, 18, Math.PI),
      makeBasic(OCTO.mouth)
    );
    mouth.position.set(0, -0.1, R * 0.82);
    mouth.rotation.z = Math.PI;
    this.head.add(mouth);

    // A tiny bump on top — kept as `this.antenna` for the inherited idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, R * 0.92, 0);
    const bump = new THREE.Mesh(new THREE.SphereGeometry(0.06, 12, 12), makeMaterial(OCTO.body, 0.5));
    bump.position.y = 0.03;
    this.antenna.add(bump);
    this.head.add(this.antenna);
  }

  // Six waving tentacles — each a chain of 5 tapering purple spheres that follow an anchor
  // near the bottom of the head with a fixed-spacing constraint plus a per-segment curl.
  buildTentacles() {
    this._tentacles = [];
    const bodyMaterial = makeMaterial(OCTO.body, 0.5);
    const suckerMaterial = makeBasic(OCTO.sucker);
    const COUNT = 6;
    const SEGS = 5;
    for (let n = 0; n < COUNT; n += 1) {
      ...
      for (let i = 0; i < SEGS; i += 1) {
        const mesh = new THREE.Mesh(new THREE.SphereGeometry(r, 14, 14), bodyMaterial);
        // Spread the initial positions downward so the chain doesn't start collapsed.
        const pos = new THREE.Vector3(
          Math.cos(angle) * 0.28,
          1.0 - 0.32 - i * 0.22,
          -0.65 + Math.sin(angle) * 0.28
        );
        mesh.position.copy(pos);
        this.root.add(mesh);

        // Tiny sucker dot on the front of each segment.
        const sucker = new THREE.Mesh(new THREE.SphereGeometry(r * 0.32, 8, 8), suckerMaterial);
        sucker.position.set(0, 0, r * 0.85);
        mesh.add(sucker);

        segs.push({ mesh, pos });
        r *= 0.78;
      }
      this._tentacles.push({ segs, angle });
    }
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyOcto tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyOcto waiting for your body pose";
  ...
  // Hide the humanoid rig, then drag each tentacle chain behind an anchor near the bottom
  // of the head with a fixed-spacing follow constraint plus a per-segment sine curl.
  syncAttachedModel(delta) {
    ...
    const spacing = 0.22;
    const a = this.head.position;
    this._tentacles.forEach(({ segs, angle }) => {
      let lead = this._lead.set(
        a.x + Math.cos(angle) * 0.28,
        a.y - 0.32,
        a.z + Math.sin(angle) * 0.28
      );
      segs.forEach((seg, i) => {
        const dir = this._dir.subVectors(seg.pos, lead);
        const d = dir.length();
        if (d > spacing) {
          dir.multiplyScalar(spacing / d);
          seg.pos.copy(lead).add(dir);
        }
        seg.mesh.position.copy(seg.pos);
        // Per-segment sine curl (display only).
        seg.mesh.position.x += Math.sin(t * 2 + i * 0.7 + angle) * 0.04;
        lead = seg.pos;
      });
    });
  }
