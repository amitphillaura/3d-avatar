...
// Fire elemental — dark glowing coal body wreathed in flickering flame tongues.
const EMBER = { coal: 0x2a0e06, ember: 0xff5a1e, flame: 0xff8a1e, flameTip: 0xffe14d, eye: 0xffe14d };

const BONE_STYLE = {
  collar: { radius: 0.055, color: EMBER.coal },
  hip: { radius: 0.06, color: EMBER.coal },
  arm: { radius: 0.045, color: EMBER.coal },
  leg: { radius: 0.052, color: EMBER.coal },
  side: { radius: 0.04, color: EMBER.coal }
};
...
// Self-lit flame tongue — glowing, translucent cone that flickers each frame.
...
export class MushyEmber {
  constructor(mount, metaElement, options = {}) {
    ...
    this.scene.background = new THREE.Color(0x140805);
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(EMBER.ember),
      right: this.buildHandRig(EMBER.ember)
    };
    this.resize();
  ...
  // Build one upward-pointing flame tongue and register it with a random flicker phase.
  // `parent` is the torso/head so the tongue rides along with the body.
  addFlame(parent, position, scale = 1) {
    const mesh = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.42, 6), makeFlame(EMBER.flame));
    mesh.position.copy(position);
    mesh.scale.setScalar(scale);
    parent.add(mesh);
    this._flames.push({ mesh, phase: Math.random() * Math.PI * 2 });
    return mesh;
  }

  createRig() {
    const jointGeometry = new THREE.SphereGeometry(0.078, 20, 20);
    const jointMaterial = makeMaterial(EMBER.coal, 0.5);
    jointMaterial.emissive = new THREE.Color(EMBER.ember);
    ...
    // Per-frame flame tongues live here; created below for torso + head.
    this._flames = [];

    // Dark-ember end-caps so wrists read as hands and ankles as feet.
    const handGeometry = new THREE.SphereGeometry(0.12, 18, 18);
    const footGeometry = new THREE.SphereGeometry(0.13, 18, 18);
    const capMaterial = makeMaterial(EMBER.coal, 0.5);
    capMaterial.emissive = new THREE.Color(EMBER.ember);
    capMaterial.emissiveIntensity = 0.45;
    this.caps = {
    ...
    const neckMaterial = makeMaterial(EMBER.coal, 0.5);
    neckMaterial.emissive = new THREE.Color(EMBER.ember);
    neckMaterial.emissiveIntensity = 0.4;
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      material.emissive = new THREE.Color(EMBER.ember);
    ...
    // Dark glowing coal torso with a strong ember emissive.
    const torsoMaterial = makeMaterial(EMBER.coal, 0.8);
    torsoMaterial.emissive = new THREE.Color(EMBER.ember);
    torsoMaterial.emissiveIntensity = 0.7;
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 32, 32), torsoMaterial);
    this.torso.scale.set(0.85, 1.15, 0.36);
    this.root.add(this.torso);

    // Dark glowing coal head.
    const headMaterial = makeMaterial(EMBER.coal, 0.8);
    headMaterial.emissive = new THREE.Color(EMBER.ember);
    headMaterial.emissiveIntensity = 0.55;
    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.32, 32, 32), headMaterial);
    this.root.add(this.head);

    // Glowing yellow eyes.
    const eyeMaterial = new THREE.MeshBasicMaterial({ color: EMBER.eye });
    ...
    // THE TECHNIQUE — flame tongues that flicker scale + cycle emissive color.
    // A ring around/atop the torso, riding along as children of the torso.
    [
      [0, 0.85, 0],
      [0.4, 0.55, 0.12],
      [-0.4, 0.55, 0.12],
      ...
      this.addFlame(this.torso, new THREE.Vector3(x, y, z), 0.9 + Math.random() * 0.3);
    });

    // A couple of flame tongues atop the head.
    this.addFlame(this.head, new THREE.Vector3(-0.16, 0.3, 0.02), 0.7);
    this.addFlame(this.head, new THREE.Vector3(0.16, 0.3, 0.02), 0.7);

    // Top-most flame on the head = `this.antenna` (a Group with one tongue) for the idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.33, 0);
    this.head.add(this.antenna);
    this.addFlame(this.antenna, new THREE.Vector3(0, 0.1, 0), 1.0);
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyEmber tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyEmber waiting for your body pose";
  ...
  // THE TECHNIQUE — drive the flame tongues each frame: flicker their scale and cycle
  // the emissive color between orange and yellow so the body looks like it's burning.
  syncAttachedModel(delta) {
    if (!this._flames) return;
    const t = (performance.now() - this.startedAt) / 1000;
    this._flames.forEach(({ mesh, phase }) => {
      mesh.scale.y = 0.7 + 0.5 * Math.abs(Math.sin(t * 8 + phase));
      mesh.scale.x = mesh.scale.z = 0.85 + 0.15 * Math.sin(t * 10 + phase);
      mesh.material.emissive.setRGB(1, 0.45 + 0.35 * Math.abs(Math.sin(t * 6 + phase)), 0.12);
    });
  }
