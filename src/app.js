...
// Queue sidebar
function initQueueSidebar() {
  const sidebar = document.getElementById("queue-sidebar");
  if (!sidebar) return;

  const toggleBtn = document.getElementById("queue-sidebar-toggle");
  const closeBtn = document.getElementById("queue-sidebar-close");

  toggleBtn?.addEventListener("click", () => {
    sidebar.classList.toggle("queue-sidebar--open");
  });
  closeBtn?.addEventListener("click", () => {
    sidebar.classList.remove("queue-sidebar--open");
  });
  ...
  function renderQueueJobs(jobs) {
    ...
    if (!listEl) return;
    if (!jobs?.length) {
      listEl.innerHTML = '<p class="queue-empty">Queue is empty</p>';
      return;
    }
    listEl.innerHTML = jobs.map(job => {
      const icon = job.status === "ready" ? "✓" : job.status === "processing" ? "⟳" : job.status === "failed" ? "✗" : "•";
      const pct = job.progress != null && job.status !== "ready" ? ` ${job.progress}%` : "";
      return `<div class="queue-item queue-item--${job.status}" data-id="${job.id}">
        <span class="queue-icon">${icon}</span>
        <span class="queue-name">${job.filename || job.id}</span>
        <span class="queue-status">${job.status}${pct}</span>
      </div>`;
    }).join("");

    listEl.querySelectorAll(".queue-item--ready").forEach(el => {
      el.style.cursor = "pointer";
      el.addEventListener("click", () => {
        const id = el.dataset.id;
        window.dispatchEvent(new CustomEvent("studio:open-video", { detail: { videoId: id } }));
      });
    });
  }
...
window.addEventListener("load", initQueueSidebar);
