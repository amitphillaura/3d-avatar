...
// Clockwork automaton — brass & copper, with exposed gears that spin (faster when moving).
const COG = { brass: 0xb8860b, copper: 0xc8773a, iron: 0x3a322a, glass: 0x9fe8ff, gold: 0xffd24a };

const BONE_STYLE = {
  collar: { radius: 0.09, color: COG.brass },
  hip: { radius: 0.1, color: COG.iron },
  arm: { radius: 0.08, color: COG.copper },
  leg: { radius: 0.1, color: COG.brass },
  side: { radius: 0.085, color: COG.iron }
};
...
// Brushed brass/copper plating.
function makeBrass(color, roughness = 0.4) {
  return new THREE.MeshStandardMaterial({
    color,
    roughness,
    metalness: 0.9,
...
// A toothed gear (disc + rim teeth + dark hub), built facing +z so rotation.z spins it.
function makeGear(radius, teeth, color) {
  const gear = new THREE.Group();
  const disc = new THREE.Mesh(
    new THREE.CylinderGeometry(radius, radius, radius * 0.3, Math.max(10, teeth)),
    makeBrass(color)
  );
  disc.rotation.x = Math.PI / 2;
  gear.add(disc);
  const toothMaterial = makeBrass(color);
  for (let i = 0; i < teeth; i += 1) {
    const angle = (i / teeth) * Math.PI * 2;
    const tooth = new THREE.Mesh(new THREE.BoxGeometry(radius * 0.24, radius * 0.24, radius * 0.3), toothMaterial);
    tooth.position.set(Math.cos(angle) * radius * 1.04, Math.sin(angle) * radius * 1.04, 0);
    tooth.rotation.z = angle;
    gear.add(tooth);
  }
  const hub = new THREE.Mesh(new THREE.CylinderGeometry(radius * 0.3, radius * 0.3, radius * 0.38, 12), makeBrass(COG.iron));
...
export class MushyCog {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(COG.copper),
      right: this.buildHandRig(COG.copper)
    };
    this.resize();
  ...
  createRig() {
    ...
    const jointGeometry = new THREE.SphereGeometry(0.09, 14, 14);
    const jointMaterial = makeBrass(COG.iron, 0.45);
    ...
    // Riveted brass fists, iron boots.
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.24, 0.24), makeBrass(COG.copper)),
      rightWrist: new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.24, 0.24), makeBrass(COG.copper)),
      leftAnkle: new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.18, 0.42), makeBrass(COG.iron)),
      rightAnkle: new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.18, 0.42), makeBrass(COG.iron))
    };
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const mesh = new THREE.Mesh(geometry, makeBrass(style.color));
      mesh.visible = false;
      this.root.add(mesh);
      this.bones.push({ from, to, mesh });
    });

    // Brass barrel torso with exposed gears front and back.
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 24, 24), makeBrass(COG.brass));
    this.torso.scale.set(0.9, 1.1, 0.62);
    this.root.add(this.torso);

    const chestGear = makeGear(0.3, 12, COG.copper);
    chestGear.position.set(0, 0.06, 0.55);
    this.torso.add(chestGear);
    this._gears.push({ mesh: chestGear, dir: 1 });

    const smallGear = makeGear(0.16, 10, COG.brass);
    smallGear.position.set(0.34, -0.16, 0.52);
    this.torso.add(smallGear);
    this._gears.push({ mesh: smallGear, dir: -1 });

    const backGear = makeGear(0.24, 12, COG.brass);
    backGear.position.set(0, 0.1, -0.55);
    this.torso.add(backGear);
    this._gears.push({ mesh: backGear, dir: -1 });

    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.32, 24, 24), makeBrass(COG.brass));
    this.root.add(this.head);
    this.buildCogHead();
  }

  buildCogHead() {
    const eyeMaterial = new THREE.MeshStandardMaterial({ color: COG.glass, emissive: COG.glass, emissiveIntensity: 0.6, roughness: 0.3 });
    const eyeGeometry = new THREE.SphereGeometry(0.05, 12, 12);
    this.leftEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.leftEye.position.set(-0.12, 0.05, 0.29);
    this.rightEye.position.set(0.12, 0.05, 0.29);
    this.head.add(this.leftEye, this.rightEye);

    // Monocle over the right eye.
    const ring = new THREE.Mesh(new THREE.TorusGeometry(0.09, 0.02, 10, 22), makeBrass(COG.gold));
    ring.position.set(0.12, 0.05, 0.3);
    this.head.add(ring);
    const glass = new THREE.Mesh(
      new THREE.CircleGeometry(0.08, 18),
      new THREE.MeshStandardMaterial({ color: COG.glass, transparent: true, opacity: 0.35, roughness: 0.1, metalness: 0.2 })
    );
    glass.position.set(0.12, 0.05, 0.29);
    this.head.add(glass);

    // Curled brass mustache.
    const stacheMaterial = makeBrass(0x2a1d10);
    [-1, 1].forEach((s) => {
      const half = new THREE.Mesh(new THREE.SphereGeometry(0.07, 12, 12), stacheMaterial);
      half.position.set(s * 0.08, -0.12, 0.29);
      half.scale.set(1.3, 0.5, 0.5);
      this.head.add(half);
    });

    // Top valve + gear — kept as `this.antenna` for the inherited idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.3, 0);
    const pipe = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.04, 0.16, 10), makeBrass(COG.iron));
    pipe.position.y = 0.08;
    const topGear = makeGear(0.1, 8, COG.gold);
    topGear.position.y = 0.22;
    this.antenna.add(pipe, topGear);
    this._gears.push({ mesh: topGear, dir: 1 });
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyCog tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyCog waiting for your body pose";
  ...
  // Spin every gear continuously, faster the more the body is moving.
  syncAttachedModel(delta) {
    if (!this._gears) return;
    const dt = Math.max(delta || 1 / 60, 1 / 120);
    let extra = 0;
    if (this.torso) {
      extra = Math.min(this.torso.position.distanceTo(this._prevPos) / dt, 6);
      this._prevPos.copy(this.torso.position);
    }
    const rate = 1 + extra * 1.4;
    this._gears.forEach((g) => {
      g.mesh.rotation.z += g.dir * rate * dt;
    });
  }
