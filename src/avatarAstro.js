...
// Chunky astronaut — white pressure suit, orange trim, a glass bubble helmet.
const ASTRO = {
  suit: 0xeef2f7,
  suitShade: 0xcdd6e2,
  accent: 0xff7a3c, // boots + helmet rim
  glass: 0xaad4ff, // helmet
  skin: 0xf3c9a6,
  panel: 0x33e1ff // glowing chest controls
};

// Thick suit limbs; arm/leg bones also get ribbed "puff" beads along their length
// (built in createRig, positioned each frame in syncAttachedModel) for the segmented look.
const BONE_STYLE = {
  collar: { radius: 0.1, color: ASTRO.suit },
  hip: { radius: 0.11, color: ASTRO.suit },
  arm: { radius: 0.09, color: ASTRO.suit },
  leg: { radius: 0.1, color: ASTRO.suit },
  side: { radius: 0.09, color: ASTRO.suit }
};
...
export class MushyAstro {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(ASTRO.suit),
      right: this.buildHandRig(ASTRO.suit)
    };
    this.resize();
  ...
  createRig() {
    // Big white suit puffs at every joint.
    const jointGeometry = new THREE.SphereGeometry(0.1, 16, 16);
    const jointMaterial = makeMaterial(ASTRO.suit, 0.6);
    ...
    // White glove hands, orange boots.
    const handMaterial = makeMaterial(ASTRO.suit, 0.55);
    const bootMaterial = makeMaterial(ASTRO.accent, 0.5);
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.SphereGeometry(0.15, 16, 16), handMaterial),
      rightWrist: new THREE.Mesh(new THREE.SphereGeometry(0.15, 16, 16), handMaterial),
      leftAnkle: new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.2, 0.42), bootMaterial),
      rightAnkle: new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.2, 0.42), bootMaterial)
    };
    ...
    this.neck = new THREE.Mesh(
      new THREE.CylinderGeometry(neckStyle.radius * 0.85, neckStyle.radius, 1, 14),
      makeMaterial(ASTRO.suit, 0.6)
    );
    this.neck.visible = false;
    this.root.add(this.neck);

    // Suit limbs, with ribbed beads along arm/leg bones (the segmented-limb technique).
    this._ribs = [];
    const ribMaterial = makeMaterial(ASTRO.suitShade, 0.5);
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeMaterial(style.color, 0.55);
      const mesh = new THREE.Mesh(geometry, material);
      mesh.visible = false;
      this.root.add(mesh);
      this.bones.push({ from, to, mesh });
      if (variant === "arm" || variant === "leg") {
        [0.35, 0.65].forEach((frac) => {
          const bead = new THREE.Mesh(new THREE.SphereGeometry(style.radius * 1.5, 14, 14), ribMaterial);
          bead.visible = false;
          this.root.add(bead);
          this._ribs.push({ bead, boneMesh: mesh, from, to, frac });
        });
      }
    });

    // Padded white suit torso, with a life-support backpack and a glowing chest panel.
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 28, 28), makeMaterial(ASTRO.suit, 0.6));
    this.torso.scale.set(0.95, 1.1, 0.72);
    this.root.add(this.torso);

    const backpack = new THREE.Mesh(new THREE.BoxGeometry(0.74, 0.92, 0.4), makeMaterial(ASTRO.suitShade, 0.5));
    backpack.position.set(0, 0.05, -0.62);
    this.torso.add(backpack);

    const chestPanel = new THREE.Mesh(
      new THREE.BoxGeometry(0.34, 0.22, 0.12),
      new THREE.MeshStandardMaterial({ color: ASTRO.panel, emissive: ASTRO.panel, emissiveIntensity: 0.8, roughness: 0.4 })
    );
    chestPanel.position.set(0, 0.06, 0.62);
    this.torso.add(chestPanel);

    // Head: a little face inside a glass bubble helmet.
    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.3, 28, 28), makeMaterial(ASTRO.skin, 0.7));
    this.root.add(this.head);
    this.buildAstroHead();
  }

  buildAstroHead() {
    const eyeMaterial = new THREE.MeshBasicMaterial({ color: 0x1a1410 });
    const eyeGeometry = new THREE.SphereGeometry(0.042, 12, 12);
    this.leftEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.leftEye.position.set(-0.1, 0.04, 0.27);
    this.rightEye.position.set(0.1, 0.04, 0.27);
    this.head.add(this.leftEye, this.rightEye);

    const smile = new THREE.Mesh(
      new THREE.TorusGeometry(0.08, 0.018, 8, 20, Math.PI),
      new THREE.MeshBasicMaterial({ color: 0x7a4a3a })
    );
    smile.position.set(0, -0.07, 0.27);
    smile.rotation.z = Math.PI;
    this.head.add(smile);

    // Glass bubble helmet around the face.
    const helmet = new THREE.Mesh(
      new THREE.SphereGeometry(0.46, 28, 28),
    ...
    // Orange neck ring at the helmet collar.
    const rim = new THREE.Mesh(new THREE.TorusGeometry(0.33, 0.05, 12, 28), makeMaterial(ASTRO.accent, 0.4));
    rim.rotation.x = Math.PI / 2;
    rim.position.y = -0.32;
    this.head.add(rim);

    // Helmet antenna — kept as `this.antenna` for the inherited idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0.3, 0.32, 0);
    const rod = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.22, 8), makeMaterial(ASTRO.suitShade, 0.5));
    rod.position.y = 0.11;
    const tip = new THREE.Mesh(
      new THREE.SphereGeometry(0.03, 10, 10),
      new THREE.MeshStandardMaterial({ color: ASTRO.accent, emissive: ASTRO.accent, emissiveIntensity: 0.7 })
    );
    tip.position.y = 0.24;
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyAstro tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyAstro waiting for your body pose";
  ...
  // Slide the ribbed suit beads along each visible arm/leg bone every frame.
  syncAttachedModel() {
    this._ribs.forEach(({ bead, boneMesh, from, to, frac }) => {
      bead.visible = boneMesh.visible;
      if (!bead.visible) return;
      bead.position.lerpVectors(this.points.get(from), this.points.get(to), frac);
    });
  }
