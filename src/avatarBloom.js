...
// Flower fairy — a blossom head ringed with petals, leafy green limbs.
const BLOOM = { stem: 0x6abf5a, petal: 0xff8fc8, center: 0xffd54a, eye: 0x3a2630, cheek: 0xff9ec4, mouth: 0x7a3550 };

const BONE_STYLE = {
  collar: { radius: 0.07, color: BLOOM.stem },
  hip: { radius: 0.08, color: BLOOM.stem },
  arm: { radius: 0.06, color: BLOOM.stem },
  leg: { radius: 0.08, color: BLOOM.stem },
  side: { radius: 0.07, color: BLOOM.stem }
};
...
export class MushyBloom {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(BLOOM.stem),
      right: this.buildHandRig(BLOOM.stem)
    };
    this.resize();
  ...
  createRig() {
    const jointGeometry = new THREE.SphereGeometry(0.075, 16, 16);
    const jointMaterial = makeMaterial(BLOOM.stem, 0.6);
    ...
    // Leaf hands and feet (flattened).
    const leafMaterial = makeMaterial(BLOOM.stem, 0.6);
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.SphereGeometry(0.13, 14, 14), leafMaterial),
      rightWrist: new THREE.Mesh(new THREE.SphereGeometry(0.13, 14, 14), leafMaterial),
      leftAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.14, 14, 14), leafMaterial),
      rightAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.14, 14, 14), leafMaterial)
    };
    this.caps.leftWrist.scale.set(1.4, 0.5, 1);
    this.caps.rightWrist.scale.set(1.4, 0.5, 1);
    this.caps.leftAnkle.scale.set(1.1, 0.5, 1.4);
    this.caps.rightAnkle.scale.set(1.1, 0.5, 1.4);
    ...
    this.neck = new THREE.Mesh(
      new THREE.CylinderGeometry(neckStyle.radius * 0.8, neckStyle.radius, 1, 12),
      makeMaterial(BLOOM.stem, 0.6)
    );
    ...
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 26, 26), makeMaterial(BLOOM.stem, 0.6));
    this.torso.scale.set(0.9, 1.12, 0.72);
    this.root.add(this.torso);

    // Yellow flower center as the head.
    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.26, 24, 24), makeMaterial(BLOOM.center, 0.55));
  ...
  buildBloomHead() {
    const R = 0.26;
    // Radial ring of petals around the blossom face (the radial-instance technique).
    this._petals = [];
    const petalMaterial = makeMaterial(BLOOM.petal, 0.5);
    const PETALS = 9;
    for (let i = 0; i < PETALS; i += 1) {
      const angle = (i / PETALS) * Math.PI * 2;
      const petal = new THREE.Mesh(new THREE.SphereGeometry(0.16, 12, 12), petalMaterial);
      petal.scale.set(0.45, 1.05, 0.22);
      petal.position.set(Math.cos(angle) * 0.36, Math.sin(angle) * 0.36, -0.05);
      petal.rotation.z = angle - Math.PI / 2;
      this.head.add(petal);
      this._petals.push({ petal, angle });
    }

    const eyeMaterial = new THREE.MeshBasicMaterial({ color: BLOOM.eye });
    const eyeGeometry = new THREE.SphereGeometry(0.045, 12, 12);
    this.leftEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.leftEye.position.set(-0.09, 0.03, R * 0.92);
    this.rightEye.position.set(0.09, 0.03, R * 0.92);
    this.head.add(this.leftEye, this.rightEye);

    const cheekMaterial = new THREE.MeshBasicMaterial({ color: BLOOM.cheek, transparent: true, opacity: 0.6 });
    [-1, 1].forEach((s) => {
      const cheek = new THREE.Mesh(new THREE.SphereGeometry(0.04, 10, 10), cheekMaterial);
      cheek.position.set(s * 0.15, -0.04, R * 0.85);
      cheek.scale.set(1.2, 0.8, 0.4);
      this.head.add(cheek);
    });

    const smile = new THREE.Mesh(
      new THREE.TorusGeometry(0.06, 0.016, 8, 18, Math.PI),
      new THREE.MeshBasicMaterial({ color: BLOOM.mouth })
    );
    smile.position.set(0, -0.08, R * 0.9);
    smile.rotation.z = Math.PI;
    this.head.add(smile);

    // Little sprout bud on top — kept as `this.antenna` for the inherited idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, R * 0.6, -0.04);
    const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.014, 0.018, 0.16, 8), makeMaterial(BLOOM.stem, 0.6));
    stem.position.y = 0.1;
    const bud = new THREE.Mesh(new THREE.SphereGeometry(0.05, 12, 12), makeMaterial(BLOOM.petal, 0.5));
    bud.position.y = 0.2;
    this.antenna.add(stem, bud);
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyBloom tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyBloom waiting for your body pose";
  ...
  // Gently breathe the petals so the blossom shimmers.
  syncAttachedModel() {
    if (!this._petals) return;
    const t = (performance.now() - this.startedAt) / 1000;
    this._petals.forEach(({ petal }, i) => {
      petal.scale.set(0.45, 1.05 * (1 + Math.sin(t * 2 + i * 0.6) * 0.1), 0.22);
    });
  }
