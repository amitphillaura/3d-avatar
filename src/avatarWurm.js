...
// Caterpillar — a head (face) with a trailing chain of striped body segments that follow
// the head with lag. The humanoid bones are hidden; the chain IS the body.
const WURM = { body: 0x7ed957, stripe: 0xffb43d, eye: 0x231a12, mouth: 0x5a2a1a };

const BONE_STYLE = {
  collar: { radius: 0.06, color: WURM.body },
  hip: { radius: 0.06, color: WURM.body },
  arm: { radius: 0.05, color: WURM.body },
  leg: { radius: 0.05, color: WURM.body },
  side: { radius: 0.05, color: WURM.body }
};
...
export class MushyWurm {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(WURM.body),
      right: this.buildHandRig(WURM.body)
    };
    this.resize();
  ...
  createRig() {
    ...
    // hidden every frame — the trailing segment chain is the caterpillar's body.
    const jointGeometry = new THREE.SphereGeometry(0.06, 10, 10);
    const jointMaterial = makeMaterial(WURM.body, 0.55);
    ...
    const capMaterial = makeMaterial(WURM.body, 0.55);
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.SphereGeometry(0.1, 10, 10), capMaterial),
      rightWrist: new THREE.Mesh(new THREE.SphereGeometry(0.1, 10, 10), capMaterial),
      leftAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.1, 10, 10), capMaterial),
      rightAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.1, 10, 10), capMaterial)
    };
    ...
    this.neck = new THREE.Mesh(
      new THREE.CylinderGeometry(neckStyle.radius, neckStyle.radius, 1, 8),
      makeMaterial(WURM.body, 0.55)
    );
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const mesh = new THREE.Mesh(geometry, makeMaterial(style.color, 0.55));
      mesh.visible = false;
      this.root.add(mesh);
      this.bones.push({ from, to, mesh });
    });

    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.4, 12, 12), makeMaterial(WURM.body, 0.55));
    this.torso.visible = false;
    this.root.add(this.torso);

    // Caterpillar face (the front of the chain).
    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.3, 24, 24), makeMaterial(WURM.body, 0.55));
  ...
  buildWurmFace() {
    ...
    const pupilMaterial = new THREE.MeshBasicMaterial({ color: WURM.eye });
    const eyes = [];
    [-1, 1].forEach((s) => {
      const eye = new THREE.Mesh(new THREE.SphereGeometry(0.1, 14, 14), eyeMaterial);
      eye.position.set(s * 0.12, 0.08, R * 0.82);
      this.head.add(eye);
      eyes.push(eye);
      const pupil = new THREE.Mesh(new THREE.SphereGeometry(0.05, 12, 12), pupilMaterial);
      pupil.position.set(s * 0.12, 0.07, R * 0.95);
      this.head.add(pupil);
    });
    this.leftEye = eyes[0];
    ...
    const smile = new THREE.Mesh(
      new THREE.TorusGeometry(0.09, 0.022, 8, 20, Math.PI),
      new THREE.MeshBasicMaterial({ color: WURM.mouth })
    );
    smile.position.set(0, -0.12, R * 0.86);
    smile.rotation.z = Math.PI;
    this.head.add(smile);

    // Two antennae — kept as `this.antenna` for the inherited idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, R * 0.8, 0);
    [-1, 1].forEach((s) => {
      const stalk = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.2, 6), makeMaterial(WURM.body, 0.55));
      stalk.position.set(s * 0.08, 0.1, 0);
      stalk.rotation.z = -s * 0.3;
      const ball = new THREE.Mesh(new THREE.SphereGeometry(0.04, 10, 10), makeMaterial(WURM.stripe, 0.5));
      ball.position.set(s * 0.13, 0.21, 0);
      this.antenna.add(stalk, ball);
    });
    this.head.add(this.antenna);
  }

  // Striped, tapering body segments stored with their own positions for the follow chain.
  buildSegments() {
    this._segments = [];
    let r = 0.24;
    for (let i = 0; i < 7; i += 1) {
      const material = makeMaterial(i % 2 === 0 ? WURM.body : WURM.stripe, 0.55);
      const seg = new THREE.Mesh(new THREE.SphereGeometry(r, 16, 16), material);
      const pos = new THREE.Vector3(0, 1.0 - i * 0.32, -0.65 - i * 0.12);
      seg.position.copy(pos);
      this.root.add(seg);
      this._segments.push({ mesh: seg, pos });
      r *= 0.92;
    }
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyWurm tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyWurm waiting for your body pose";
  ...
  // Hide the humanoid rig, then drag the segment chain behind the head with a fixed-spacing
  // follow constraint plus a side-to-side ripple.
  syncAttachedModel() {
    this.bones.forEach(({ mesh }) => {
      mesh.visible = false;
    });
    ...
    if (!this._segments || !this.head) return;

    const t = (performance.now() - this.startedAt) / 1000;
    let lead = this._lead.copy(this.head.position);
    const spacing = 0.32;
    this._segments.forEach((seg, i) => {
      ...
      seg.mesh.position.x += Math.sin(t * 3 + i * 0.8) * 0.05; // ripple (display only)
      lead = seg.pos;
    });
  }
