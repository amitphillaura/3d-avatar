...
export class MushyAvatar {
  ...
  updatePose(poseLandmarks) {
    ...
    if (visibleCount >= 5) {
      ...
      this.metaElement.textContent = `Mushy tracking ${visibleCount} body points`;
    }
  }
  ...
  animateIdle(delta) {
    ...
    if (performance.now() - this.latestTrackedAt > 900) {
  ...
  updateRig(delta) {
    ...
    const tracking = performance.now() - this.latestTrackedAt <= 900;
    if (!tracking) return;
    ...
    this.bones.forEach(({ from, to, mesh }) => {
      ...
      mesh.visible = this.activePoints.has(from) && this.activePoints.has(to) && start.distanceTo(end) > 0.05;
      setCylinderBetween(mesh, start, end);
    });

    // Hands and feet ride on the smoothed wrist/ankle joints.
    ...
    if (this.activePoints.has("nose")) {
      ...
      // Tilt the head (eyes face +Z) from the nose offset relative to the shoulders.
      ...
      // Neck bridges the shoulder line to the head.
