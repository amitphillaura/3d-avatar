...
// MushyPix — a retro 8-bit sprite living in the 3D scene. The humanoid rig is still built
// (the base loop drives it) but hidden every frame; a camera-facing THREE.Sprite billboard
// with an animated pixel-art CanvasTexture is the real character.
const PIX = { skin: 0xffd27f, body: 0x39e6ff, eye: 0x101020, arm: 0xff5fa2, leg: 0x9d6bff };

const BONE_STYLE = {
  collar: { radius: 0.02, color: PIX.body },
  hip: { radius: 0.02, color: PIX.body },
  arm: { radius: 0.02, color: PIX.body },
  leg: { radius: 0.02, color: PIX.body },
  side: { radius: 0.02, color: PIX.body }
};
...
export class MushyPix {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(0x39e6ff),
      right: this.buildHandRig(0x39e6ff)
    };
    this.resize();
  ...
  createRig() {
    // The humanoid rig is contract filler only: build joints/bones/caps/neck/torso/head as
    // tiny dim meshes, all invisible-by-default. The animated pixel sprite (below) is the
    // real MushyPix character, and syncAttachedModel re-hides these every frame.
    const jointGeometry = new THREE.SphereGeometry(0.012, 6, 6);
    const jointMaterial = makeMaterial(PIX.body, 0.55);
    ...
    // End-caps so wrists read as hands and ankles as feet (kept tiny + hidden).
    const capGeometry = new THREE.SphereGeometry(0.015, 6, 6);
    const capMaterial = makeMaterial(PIX.body, 0.55);
    this.caps = {
      leftWrist: new THREE.Mesh(capGeometry, capMaterial),
      rightWrist: new THREE.Mesh(capGeometry, capMaterial),
      leftAnkle: new THREE.Mesh(capGeometry, capMaterial),
      rightAnkle: new THREE.Mesh(capGeometry, capMaterial)
    };
    ...
    this.neck = new THREE.Mesh(
      new THREE.CylinderGeometry(neckStyle.radius, neckStyle.radius, 1, 6),
      makeMaterial(PIX.body, 0.55)
    );
    ...
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.04, 6, 6), makeMaterial(PIX.body, 0.55));
    this.torso.visible = false;
    this.root.add(this.torso);

    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.03, 6, 6), makeMaterial(PIX.body, 0.55));
    this.head.visible = false;
    this.root.add(this.head);

    const eyeMaterial = new THREE.MeshBasicMaterial({ color: PIX.eye });
    this.leftEye = new THREE.Mesh(new THREE.SphereGeometry(0.008, 6, 6), eyeMaterial);
    this.rightEye = new THREE.Mesh(new THREE.SphereGeometry(0.008, 6, 6), eyeMaterial);
    this.leftEye.visible = false;
    this.rightEye.visible = false;
    this.head.add(this.leftEye, this.rightEye);
    this.leftEye.position.set(-0.01, 0.006, 0.028);
    this.rightEye.position.set(0.01, 0.006, 0.028);

    // Tiny invisible antenna group — kept for the inherited idle wiggle (`.rotation`).
    this.antenna = new THREE.Group();
    this.antenna.visible = false;
    this.antenna.position.set(0, 0.03, 0);
    this.head.add(this.antenna);

    // THE TECHNIQUE — an animated pixel-art sprite billboard.
    this._canvas = document.createElement("canvas");
    this._canvas.width = 24;
    ...
    const mat = new THREE.SpriteMaterial({ map: this._tex, transparent: true });
    this.sprite = new THREE.Sprite(mat);
    this.sprite.scale.set(1.5, 2.0, 1);
  ...
  // Draw a blocky pixel character on the 24x32 canvas. `frame` (0 or 1) bobs the body 1px
  // and alternates the arm pixels so two frames read as a simple walk/idle animation.
  drawPixel(frame) {
    const ctx = this._ctx;
    ...
    const skin = hex(PIX.skin);
    const body = hex(PIX.body);
    const eye = hex(PIX.eye);
    const arm = hex(PIX.arm);
    const leg = hex(PIX.leg);

    const bob = frame ? 1 : 0; // 1px vertical bob between frames

    // Head block + two eye pixels.
    px(8, 2 + bob, 8, 8, skin);
    px(10, 5 + bob, 2, 2, eye);
    px(13, 5 + bob, 2, 2, eye);

    // Body block.
    px(7, 10 + bob, 10, 10, body);

    // Arms — alternate which arm is raised so the two frames differ.
    if (frame) {
      px(4, 9 + bob, 3, 6, arm);
      px(17, 12 + bob, 3, 6, arm);
    } else {
      px(4, 12 + bob, 3, 6, arm);
      px(17, 9 + bob, 3, 6, arm);
    }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyPix tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyPix waiting for your body pose";
  ...
  // Hide every humanoid mesh each frame, anchor the billboard sprite to the body, and
  // flip between the two pixel-art frames a few times a second for the walk/idle animation.
  syncAttachedModel(delta) {
    ...
    // THREE.Sprite auto-faces the camera — that's the billboard.
    ...
    const now = performance.now();
    if (now - this._frameAt > 300) {
      this._frameAt = now;
      this._frame ^= 1;
      this.drawPixel(this._frame);
      this._tex.needsUpdate = true;
    }
  }
