...
// Hologram / Tron being — additive glowing wireframe lines, no solid surfaces.
const NEON = {
  cyan: 0x39e6ff,
  magenta: 0xff3df0,
  lime: 0x8cff3d,
...
const BONE_STYLE = {
  collar: { radius: 0.06, color: NEON.lime },
  hip: { radius: 0.07, color: NEON.cyan },
  arm: { radius: 0.05, color: NEON.magenta },
  leg: { radius: 0.06, color: NEON.cyan },
  side: { radius: 0.05, color: NEON.cyan }
};
...
// Glowing wireframe — additive blending makes overlapping lines bloom on the dark bg.
function makeNeon(color) {
...
// Solid additive glow (for bright eyes / core).
function neonSolid(color, opacity = 0.9) {
  return new THREE.MeshBasicMaterial({
    color,
    transparent: true,
    opacity,
...
export class MushyNeon {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(NEON.magenta),
      right: this.buildHandRig(NEON.cyan)
    };
    this.resize();
  ...
  createRig() {
    // Collect every glowing material so syncAttachedModel can flicker them together.
    this._neonMats = [];
    const neon = (color) => {
      const material = makeNeon(color);
      this._neonMats.push(material);
      return material;
    };

    const jointGeometry = new THREE.SphereGeometry(0.085, 10, 8);
    const jointMaterial = neon(NEON.cyan);
    ...
    const handMaterial = neon(NEON.magenta);
    const footMaterial = neon(NEON.cyan);
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.SphereGeometry(0.13, 10, 8), handMaterial),
      rightWrist: new THREE.Mesh(new THREE.SphereGeometry(0.13, 10, 8), handMaterial),
      leftAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.14, 10, 8), footMaterial),
      rightAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.14, 10, 8), footMaterial)
    };
    ...
    this.neck = new THREE.Mesh(
      new THREE.CylinderGeometry(neckStyle.radius * 0.7, neckStyle.radius, 1, 8),
      neon(NEON.lime)
    );
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const mesh = new THREE.Mesh(geometry, neon(style.color));
      mesh.visible = false;
      this.root.add(mesh);
      this.bones.push({ from, to, mesh });
    });

    // Wireframe torso shell with a bright additive core.
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 18, 14), neon(NEON.cyan));
    this.torso.scale.set(0.9, 1.1, 0.6);
    this.root.add(this.torso);
    const core = new THREE.Mesh(new THREE.SphereGeometry(0.16, 16, 16), neonSolid(NEON.white, 0.85));
    this.torso.add(core);

    // Wireframe head with glowing eyes.
    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.33, 18, 14), neon(NEON.cyan));
    this.root.add(this.head);

    const eyeGeometry = new THREE.SphereGeometry(0.05, 12, 12);
    const eyeMaterial = neonSolid(NEON.white, 0.95);
    this.leftEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.leftEye.position.set(-0.11, 0.06, 0.29);
    this.rightEye.position.set(0.11, 0.06, 0.29);
    this.head.add(this.leftEye, this.rightEye);

    // Antenna line + tip — kept as `this.antenna` for the inherited idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.3, 0);
    const rod = new THREE.Mesh(new THREE.CylinderGeometry(0.01, 0.01, 0.34, 6), neon(NEON.magenta));
    rod.position.y = 0.17;
    const tip = new THREE.Mesh(new THREE.SphereGeometry(0.04, 12, 12), neonSolid(NEON.magenta, 0.95));
    tip.position.y = 0.36;
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyNeon tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyNeon waiting for your body pose";
  ...
  // Hologram flicker: shimmer the wireframe opacity, with occasional brief dips.
  syncAttachedModel() {
    if (!this._neonMats) return;
    const t = performance.now() / 1000;
    const dip = Math.sin(t * 6.1) > 0.93 ? -0.28 : 0;
    const opacity = Math.max(0.2, Math.min(0.95, 0.6 + Math.sin(t * 28) * 0.05 + dip));
    this._neonMats.forEach((m) => {
      m.opacity = opacity;
    });
  }
