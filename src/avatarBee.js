...
// Chubby bumblebee — yellow striped body, little black legs, fast translucent buzzing wings.
const BEE = {
  yellow: 0xffd23f,
  black: 0x1a1a1a,
  wing: 0xcfe8ff, // translucent membrane
  eye: 0x101014,
  mouth: 0x3a1a1a,
  cheek: 0xff9ec4
};
...
const BONE_STYLE = {
  collar: { radius: 0.06, color: BEE.black },
  hip: { radius: 0.065, color: BEE.black },
  arm: { radius: 0.05, color: BEE.black },
  leg: { radius: 0.055, color: BEE.black },
  side: { radius: 0.05, color: BEE.black }
};
...
// Translucent, fast-flapping wing membrane — high frequency makes it read as a buzz blur.
function makeWingMaterial(color) {
  return new THREE.MeshStandardMaterial({
    color,
    transparent: true,
    opacity: 0.5,
...
export class MushyBee {
  constructor(mount, metaElement, options = {}) {
    ...
    this.scene.background = new THREE.Color(0x12100a);
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(BEE.black),
      right: this.buildHandRig(BEE.black)
    };
    this.resize();
  ...
  createRig() {
    const jointGeometry = new THREE.SphereGeometry(0.07, 16, 16);
    const jointMaterial = makeMaterial(BEE.black, 0.5);
    ...
    // Little black bee hands/feet.
    const limbGeometry = new THREE.SphereGeometry(0.12, 16, 16);
    const limbMaterial = makeMaterial(BEE.black, 0.45);
    this.caps = {
      leftWrist: new THREE.Mesh(limbGeometry, limbMaterial),
      rightWrist: new THREE.Mesh(limbGeometry, limbMaterial),
      leftAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.13, 16, 16), limbMaterial),
      rightAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.13, 16, 16), limbMaterial)
    };
    ...
    // Round yellow body with black stripe bands and a little black stinger.
    this.torso = new THREE.Mesh(
      new THREE.SphereGeometry(0.82, 28, 28),
      makeMaterial(BEE.yellow, 0.55)
    );
    this.torso.scale.set(0.92, 1.1, 0.72);
    this.root.add(this.torso);

    const stripeMaterial = makeMaterial(BEE.black, 0.5);
    [0.34, 0.0, -0.34].forEach((y) => {
      const ring = Math.sqrt(Math.max(0, 1 - (y / 0.82) ** 2)) * 0.82; // body radius at this height
      const stripe = new THREE.Mesh(new THREE.TorusGeometry(ring, 0.07, 10, 28), stripeMaterial);
      stripe.position.y = y;
      stripe.rotation.x = Math.PI / 2;
      this.torso.add(stripe);
    });

    const stinger = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.34, 10), makeMaterial(BEE.black, 0.45));
    stinger.position.set(0, -0.78, -0.5);
    stinger.rotation.x = -2.4;
    this.torso.add(stinger);

    // Black head with cute face.
  ...
  buildBeeFace() {
    // Big cute eyes.
    const eyeMaterial = new THREE.MeshBasicMaterial({ color: BEE.eye });
    const shineMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff });
    const eyes = [];
    [-1, 1].forEach((s) => {
      const eye = new THREE.Mesh(new THREE.SphereGeometry(0.09, 16, 16), eyeMaterial);
      eye.position.set(s * 0.13, 0.06, 0.27);
      eye.scale.set(1, 1.15, 0.8);
      this.head.add(eye);
      eyes.push(eye);
      const shine = new THREE.Mesh(new THREE.SphereGeometry(0.028, 10, 10), shineMaterial);
      shine.position.set(s * 0.13 + 0.03, 0.1, 0.34);
      this.head.add(shine);
    });
    this.leftEye = eyes[0];
    ...
    // Pink cheeks.
    const cheekMaterial = new THREE.MeshBasicMaterial({ color: BEE.cheek, transparent: true, opacity: 0.85 });
    [-1, 1].forEach((s) => {
      const cheek = new THREE.Mesh(new THREE.SphereGeometry(0.05, 12, 12), cheekMaterial);
      cheek.position.set(s * 0.21, -0.05, 0.25);
      cheek.scale.set(1, 0.7, 0.4);
      this.head.add(cheek);
    });

    // A curved smile (a thin torus arc).
    const smile = new THREE.Mesh(
      new THREE.TorusGeometry(0.08, 0.016, 8, 16, Math.PI),
      new THREE.MeshBasicMaterial({ color: BEE.mouth })
    );
    smile.position.set(0, -0.1, 0.3);
    smile.rotation.z = Math.PI;
    this.head.add(smile);

    // Two antennae = a group of thin black stalks with little ball tips, angled out.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.28, 0.04);
    const stalkMaterial = makeMaterial(BEE.black, 0.45);
    const tipMaterial = makeMaterial(BEE.black, 0.4);
    [-1, 1].forEach((s) => {
      const stalk = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.26, 8), stalkMaterial);
      stalk.position.set(s * 0.08, 0.12, 0);
      stalk.rotation.z = -s * 0.4;
      this.antenna.add(stalk);
      const tip = new THREE.Mesh(new THREE.SphereGeometry(0.04, 10, 10), tipMaterial);
      tip.position.set(s * 0.18, 0.24, 0);
      this.antenna.add(tip);
    });
    this.head.add(this.antenna);

    // THE TECHNIQUE — two translucent buzzing wings anchored at the upper back.
    this._wings = [];
    const wingMaterial = makeWingMaterial(BEE.wing);
    [-1, 1].forEach((sign) => {
      const wing = new THREE.Mesh(new THREE.SphereGeometry(0.3, 16, 16), wingMaterial);
      wing.scale.set(1.4, 0.9, 0.1);
      this.root.add(wing);
      this._wings.push({ mesh: wing, sign });
    });
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyBee tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyBee waiting for your body pose";
  ...
  // Anchor the wings to the upper back each frame and flap them FAST so they read as a buzz blur.
  syncAttachedModel() {
    if (!this._wings) return;
    const t = (performance.now() - this.startedAt) / 1000;
    const tp = this.torso.position;
    this._wings.forEach(({ mesh, sign }) => {
      mesh.position.set(tp.x + sign * 0.15, tp.y + 0.35, tp.z - 0.2);
      mesh.rotation.z = sign * (0.3 + Math.sin(t * 40) * 0.55);
    });
  }
