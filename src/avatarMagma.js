...
// Molten lava-rock golem — dark cracked rock body veined with throbbing lava glow.
const MAGMA = { rock: 0x2a211c, lava: 0xff5a1e, lavaBright: 0xffb020, eye: 0xffb020 };

const BONE_STYLE = {
  collar: { radius: 0.07, color: MAGMA.rock },
  hip: { radius: 0.08, color: MAGMA.rock },
  arm: { radius: 0.06, color: MAGMA.rock },
  leg: { radius: 0.075, color: MAGMA.rock },
  side: { radius: 0.06, color: MAGMA.rock }
};
...
// Self-lit lava material — a glowing crack vein that throbs each frame.
function makeLava(color) {
...
export class MushyMagma {
  constructor(mount, metaElement, options = {}) {
    ...
    this.scene.background = new THREE.Color(0x140a06);
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(MAGMA.rock),
      right: this.buildHandRig(MAGMA.rock)
    };
    this.resize();
  ...
  // Build one thin glowing lava-crack vein on a parent surface and register it with a
  // random throb phase. `parent` is the torso/head/limb so the crack rides along.
  addCrack(parent, position, rotation) {
    const mesh = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.22, 0.05), makeLava(MAGMA.lava));
    mesh.position.copy(position);
    mesh.rotation.set(rotation.x, rotation.y, rotation.z);
    parent.add(mesh);
    this._cracks.push({ mesh, phase: Math.random() * Math.PI * 2 });
    return mesh;
  }

  createRig() {
    // Chunky rock joints — dark, faceted lava-rock spheres.
    const jointGeometry = new THREE.SphereGeometry(0.095, 8, 6);
    const jointMaterial = makeMaterial(MAGMA.rock, 0.95);
    ...
    // Glowing lava-crack veins live here; populated below across torso + head.
    this._cracks = [];

    // Rock end-caps so wrists read as hands and ankles as feet.
    const handGeometry = new THREE.SphereGeometry(0.14, 8, 6);
    const footGeometry = new THREE.SphereGeometry(0.15, 8, 6);
    const capMaterial = makeMaterial(MAGMA.rock, 0.95);
    this.caps = {
    ...
    this.neck = new THREE.Mesh(
      new THREE.CylinderGeometry(neckStyle.radius * 0.7, neckStyle.radius * 0.9, 1, 6),
      makeMaterial(MAGMA.rock, 0.95)
    );
    this.neck.visible = false;
    this.root.add(this.neck);

    // Faceted rock limbs (radialSegments 5 for chunky angular rock).
    BONES.forEach(([from, to, variant]) => {
      const style = BONE_STYLE[variant];
      const geometry = new THREE.CylinderGeometry(style.radius, style.radius, 1, 5);
      const material = makeMaterial(style.color, 0.95);
    ...
    // Dark cracked rock torso.
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 12, 10), makeMaterial(MAGMA.rock, 0.95));
    this.torso.scale.set(0.85, 1.15, 0.5);
    this.root.add(this.torso);

    // Dark cracked rock head.
    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.32, 12, 10), makeMaterial(MAGMA.rock, 0.95));
    this.root.add(this.head);

    // Glowing yellow eyes (self-lit).
    const eyeMaterial = new THREE.MeshBasicMaterial({ color: MAGMA.eye });
    this.leftEye = new THREE.Mesh(new THREE.SphereGeometry(0.05, 12, 12), eyeMaterial);
    this.rightEye = new THREE.Mesh(new THREE.SphereGeometry(0.05, 12, 12), eyeMaterial);
    ...
    // THE TECHNIQUE — lava cracks. Thin glowing vein strips strewn across the rock
    // surface of torso and head; they throb and brighten in syncAttachedModel.
    [
      [new THREE.Vector3(0.12, 0.5, 0.6), new THREE.Vector3(0, 0, 0.5)],
      [new THREE.Vector3(-0.2, 0.2, 0.6), new THREE.Vector3(0.3, 0, -0.4)],
      [new THREE.Vector3(0.28, -0.1, 0.5), new THREE.Vector3(0, 0, 1.1)],
      [new THREE.Vector3(-0.18, -0.35, 0.55), new THREE.Vector3(0, 0, 0.2)],
      [new THREE.Vector3(0.0, 0.05, 0.62), new THREE.Vector3(0, 0, -0.6)],
      [new THREE.Vector3(0.22, 0.35, 0.55), new THREE.Vector3(0, 0, 0.8)]
    ].forEach(([pos, rot]) => this.addCrack(this.torso, pos, rot));

    [
      [new THREE.Vector3(0.08, 0.15, 0.27), new THREE.Vector3(0, 0, 0.5)],
      [new THREE.Vector3(-0.12, -0.05, 0.27), new THREE.Vector3(0, 0, -0.7)],
      [new THREE.Vector3(0.15, -0.12, 0.24), new THREE.Vector3(0, 0, 1.0)],
      [new THREE.Vector3(-0.05, 0.2, 0.26), new THREE.Vector3(0, 0, 0.1)]
    ].forEach(([pos, rot]) => this.addCrack(this.head, pos, rot));

    // Antenna = a small glowing lava-rock spike on top of the head.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.33, 0);
    this.head.add(this.antenna);
    const spike = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.22, 5), makeMaterial(MAGMA.rock, 0.95));
    spike.position.y = 0.11;
    this.antenna.add(spike);
    this.addCrack(this.antenna, new THREE.Vector3(0, 0.12, 0.05), new THREE.Vector3(0, 0, 0));
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyMagma tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyMagma waiting for your body pose";
  ...
  // THE TECHNIQUE — throb the lava cracks: pulse each vein's emissive intensity and shift
  // its color toward bright yellow-orange so the rock body looks molten and alive.
  syncAttachedModel(delta) {
    if (!this._cracks) return;
    const t = (performance.now() - this.startedAt) / 1000;
    this._cracks.forEach(({ mesh, phase }) => {
      mesh.material.emissiveIntensity = 0.6 + 0.5 * Math.abs(Math.sin(t * 1.5 + phase));
      mesh.material.emissive.setRGB(1, 0.35 + 0.3 * Math.abs(Math.sin(t * 1.5 + phase)), 0.08);
    });
  }
