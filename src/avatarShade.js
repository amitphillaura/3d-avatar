...
// Shadow creature — near-black silhouette, glowing eyes, shedding smoky wisps.
const SHADE = { body: 0x0b0b14, eye: 0xb07bff, smoke: 0x5a3fa0 };

const BONE_STYLE = {
  collar: { radius: 0.06, color: SHADE.body },
  hip: { radius: 0.07, color: SHADE.body },
  arm: { radius: 0.055, color: SHADE.body },
  leg: { radius: 0.065, color: SHADE.body },
  side: { radius: 0.055, color: SHADE.body }
};
...
// Self-lit additive glow (eyes, smoke wisps).
function makeShadeGlow(color, opacity = 0.95) {
...
export class MushyShade {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(SHADE.body),
      right: this.buildHandRig(SHADE.body)
    };
    this.resize();
  ...
  createRig() {
    const jointGeometry = new THREE.SphereGeometry(0.082, 16, 16);
    const jointMaterial = makeMaterial(SHADE.body, 0.95);
    ...
    const capMaterial = makeMaterial(SHADE.body, 0.95);
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.SphereGeometry(0.13, 14, 14), capMaterial),
      rightWrist: new THREE.Mesh(new THREE.SphereGeometry(0.13, 14, 14), capMaterial),
      leftAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.14, 14, 14), capMaterial),
      rightAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.14, 14, 14), capMaterial)
    };
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeMaterial(style.color, 0.9);
    ...
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 26, 26), makeMaterial(SHADE.body, 0.92));
    this.torso.scale.set(0.9, 1.12, 0.6);
    this.root.add(this.torso);

    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.34, 26, 26), makeMaterial(SHADE.body, 0.92));
    this.root.add(this.head);

    // Big glowing eyes.
    const eyeMaterial = makeShadeGlow(SHADE.eye);
    const eyeGeometry = new THREE.SphereGeometry(0.07, 14, 14);
    this.leftEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.leftEye.position.set(-0.12, 0.05, 0.3);
    this.rightEye.position.set(0.12, 0.05, 0.3);
    this.leftEye.scale.set(1, 1.4, 1);
    this.rightEye.scale.set(1, 1.4, 1);
    this.head.add(this.leftEye, this.rightEye);

    // Jagged shadow crown.
    const spikeMaterial = makeMaterial(SHADE.body, 0.95);
    [-0.2, -0.05, 0.1, 0.24].forEach((x, i) => {
      const spike = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.22 + (i % 2) * 0.08, 5), spikeMaterial);
      spike.position.set(x, 0.32, -0.04);
      spike.rotation.z = x * 0.6;
      this.head.add(spike);
    });

    // Crown spike — kept as `this.antenna` for the inherited idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.32, 0.05);
    const tip = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.28, 5), spikeMaterial);
    tip.position.y = 0.14;
  ...
  // A pool of additive "smoke" particles shed from the body (the trailing-particle technique).
  buildWisps() {
    this._wisps = [];
    for (let i = 0; i < 16; i += 1) {
      ...
      const mesh = new THREE.Mesh(new THREE.SphereGeometry(0.13, 8, 8), material);
      mesh.visible = false;
      this.root.add(mesh);
      this._wisps.push({ mesh, vel: new THREE.Vector3(), life: 0, max: 1 });
    }
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyShade tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyShade waiting for your body pose";
  ...
  // Shed smoky wisps from the head/torso each frame: respawn dead particles at the body,
  // drift them upward, and fade + grow them as they die.
  syncAttachedModel(delta) {
    if (!this._wisps) return;
    const dt = Math.max(delta || 1 / 60, 1 / 120);
    const a = this.head?.position;
    const b = this.torso?.position;
    if (!a || !b) return;
    this._wisps.forEach((wisp) => {
      wisp.life -= dt;
      if (wisp.life <= 0) {
        const base = Math.random() < 0.55 ? a : b;
        wisp.mesh.position.set(
          base.x + (Math.random() - 0.5) * 0.5,
          base.y + (Math.random() - 0.5) * 0.5,
          base.z + (Math.random() - 0.5) * 0.3
        );
        wisp.vel.set((Math.random() - 0.5) * 0.35, 0.45 + Math.random() * 0.5, (Math.random() - 0.5) * 0.35);
        wisp.max = 0.8 + Math.random() * 0.9;
        wisp.life = wisp.max;
        wisp.mesh.visible = true;
      }
      wisp.mesh.position.addScaledVector(wisp.vel, dt);
      const f = Math.max(0, wisp.life / wisp.max);
      wisp.mesh.material.opacity = f * 0.5;
      wisp.mesh.scale.setScalar(0.5 + (1 - f) * 1.3);
    });
  }
