...
// Friendly jack-o'-lantern palette — pumpkin body with a candle-lit carved face.
const JACK = {
  pumpkin: 0xff7518,
  glow: 0xffc23a,
  stem: 0x4a7a2a,
...
const BONE_STYLE = {
  collar: { radius: 0.055, color: JACK.pumpkin },
  hip: { radius: 0.06, color: JACK.pumpkin },
  arm: { radius: 0.045, color: JACK.pumpkin },
  leg: { radius: 0.052, color: JACK.pumpkin },
  side: { radius: 0.04, color: JACK.pumpkin }
};
...
export class MushyJack {
  constructor(mount, metaElement, options = {}) {
    ...
    this.scene.background = new THREE.Color(0x140a04);
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(JACK.pumpkin),
      right: this.buildHandRig(JACK.pumpkin)
    };
    this.resize();
  ...
  createRig() {
    ...
    const jointMaterial = makeMaterial(JACK.pumpkin, 0.5);
    ...
    const handMaterial = makeMaterial(JACK.pumpkin, 0.5);
    const footMaterial = makeMaterial(JACK.pumpkin, 0.5);
    this.caps = {
    ...
    // Plump pumpkin-orange belly.
    this.torso = new THREE.Mesh(
      new THREE.SphereGeometry(0.82, 32, 32),
      makeMaterial(JACK.body, 0.6)
    );
    this.torso.scale.set(1.0, 0.92, 1.0);
    this.root.add(this.torso);

    // The PUMPKIN head — a squashed orange sphere with carved-rib grooves.
    this.headRadius = 0.34;
    this.head = new THREE.Mesh(
      new THREE.SphereGeometry(this.headRadius, 32, 32),
      makeMaterial(JACK.pumpkin, 0.6)
    );
    this.head.scale.set(1, 0.85, 1);
    this.root.add(this.head);

    // A few vertical "rib" grooves — thin darker scaled spheres around the pumpkin.
    const ribMat = makeMaterial(JACK.ribDark, 0.6);
    const ribCount = 5;
    for (let i = 0; i < ribCount; i += 1) {
      const angle = (i / ribCount) * Math.PI * 2;
      const rib = new THREE.Mesh(new THREE.SphereGeometry(this.headRadius * 0.98, 20, 20), ribMat);
      rib.scale.set(0.12, 1.02, 1.02);
      rib.position.set(Math.sin(angle) * this.headRadius * 0.92, 0, Math.cos(angle) * this.headRadius * 0.92);
    ...
    // Green stem on top, kept as `this.antenna` so the idle wiggle still sways it.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, this.headRadius * 0.82, 0);
    const stemMat = makeMaterial(JACK.stem, 0.7);
    const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.05, 0.16, 10), stemMat);
    stem.position.y = 0.08;
    this.antenna.add(stem);
    const stemCap = new THREE.Mesh(new THREE.ConeGeometry(0.045, 0.1, 10), stemMat);
    stemCap.position.y = 0.2;
    stemCap.rotation.x = 0.3;
    this.antenna.add(stemCap);
    this.head.add(this.antenna);
  }

  // THE TECHNIQUE — a glowing carved face. Emissive cut-out triangle eyes and a jagged
  // grin sit on the pumpkin's front face, lit from within by a flickering candle point
  // light. `syncAttachedModel(delta)` modulates the glow each frame so it breathes like
  // a real candle inside a jack-o'-lantern.
  buildFace() {
    const R = this.headRadius;
    const z = R * 0.9; // push cut-outs to the pumpkin front
    ...
    // Triangular eyes — cones with 3 radial segments, laid flat to face forward (point up).
    const buildEye = (sign) => {
      const eye = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.08, 3), glowMat);
      eye.rotation.x = Math.PI / 2;
      eye.position.set(sign * 0.13, 0.08, z);
      this.head.add(eye);
      return eye;
    };
    ...
    // Jagged grin — a row of small triangles alternating point up / point down.
    const teeth = 4;
    for (let i = 0; i < teeth; i += 1) {
      const tooth = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.06, 3), glowMat);
      tooth.rotation.x = Math.PI / 2;
      // alternate flipping the triangle so the grin reads as a zig-zag
      if (i % 2 === 1) tooth.rotation.y = Math.PI;
      tooth.position.set((i - (teeth - 1) / 2) * 0.085, -0.1, z);
      this.head.add(tooth);
    }

    // Candle inside the pumpkin for the lit-from-within glow.
    this._candle = new THREE.PointLight(0xff8a1e, 2.2, 4);
    this._candle.position.set(0, 0, 0);
    this.head.add(this._candle);
    this._faceMats = [glowMat];
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyJack tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyJack waiting for your body pose";
  ...
  // Per-frame candle flicker — modulates the inner point light and the carved-face
  // emissive glow so the jack-o'-lantern breathes like a real candle.
  syncAttachedModel(delta) {
    if (!this._candle) return;
    const t = (performance.now() - this.startedAt) / 1000;
    const f = 0.8 + 0.2 * Math.sin(t * 13) + 0.12 * Math.sin(t * 27);
    if (this._candle) this._candle.intensity = 2.0 * f;
    this._faceMats.forEach((m) => {
      m.emissiveIntensity = 0.9 * f;
    });
  }
