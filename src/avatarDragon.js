...
// Chunky cartoon dragon — green scales, gold horns, a flapping pair of wings and a tail.
const DRAGON = {
  body: 0x4caf50,
  belly: 0xd6e8a0,
  spike: 0xe8c84d, // horns / frills / tail tip
  wing: 0x8e44ad, // membrane (purple for contrast)
  eye: 0xfff200,
  pupil: 0x141014,
  tooth: 0xffffff
};
...
const BONE_STYLE = {
  collar: { radius: 0.09, color: DRAGON.body },
  hip: { radius: 0.1, color: DRAGON.body },
  arm: { radius: 0.08, color: DRAGON.body },
  leg: { radius: 0.1, color: DRAGON.body },
  side: { radius: 0.085, color: DRAGON.body }
};
...
export class MushyDragon {
  constructor(mount, metaElement, options = {}) {
    ...
    this.scene.background = new THREE.Color(0x0c1410);
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(DRAGON.body),
      right: this.buildHandRig(DRAGON.body)
    };
    this.resize();
  ...
  createRig() {
    const jointGeometry = new THREE.SphereGeometry(0.09, 16, 16);
    const jointMaterial = makeMaterial(DRAGON.body, 0.55);
    ...
    const clawMaterial = makeMaterial(DRAGON.body, 0.5);
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.SphereGeometry(0.14, 14, 14), clawMaterial),
      rightWrist: new THREE.Mesh(new THREE.SphereGeometry(0.14, 14, 14), clawMaterial),
      leftAnkle: new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.16, 0.4), clawMaterial),
      rightAnkle: new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.16, 0.4), clawMaterial)
    };
    ...
    this.neck = new THREE.Mesh(
      new THREE.CylinderGeometry(neckStyle.radius * 0.8, neckStyle.radius, 1, 14),
      makeMaterial(DRAGON.body, 0.55)
    );
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeMaterial(style.color, 0.5);
    ...
    // Green body with a pale belly plate.
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 28, 28), makeMaterial(DRAGON.body, 0.55));
    this.torso.scale.set(0.95, 1.1, 0.7);
    this.root.add(this.torso);
    const belly = new THREE.Mesh(new THREE.SphereGeometry(0.6, 20, 20), makeMaterial(DRAGON.belly, 0.6));
    belly.scale.set(0.8, 1.0, 0.7);
    belly.position.set(0, -0.05, 0.4);
    this.torso.add(belly);

    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.32, 28, 28), makeMaterial(DRAGON.body, 0.55));
    this.root.add(this.head);
    this.buildDragonHead();
    this.buildDragonTail();
    this.buildWings();
  }

  buildDragonHead() {
    const bodyMaterial = makeMaterial(DRAGON.body, 0.55);

    const snout = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.22, 0.32), bodyMaterial);
    snout.position.set(0, -0.08, 0.34);
    this.head.add(snout);

    const nostrilMaterial = new THREE.MeshBasicMaterial({ color: 0x143018 });
    [-1, 1].forEach((s) => {
      const nostril = new THREE.Mesh(new THREE.SphereGeometry(0.022, 8, 8), nostrilMaterial);
      nostril.position.set(s * 0.06, -0.04, 0.5);
      this.head.add(nostril);
    });

    const eyeMaterial = new THREE.MeshBasicMaterial({ color: DRAGON.eye });
    const pupilMaterial = new THREE.MeshBasicMaterial({ color: DRAGON.pupil });
    const eyes = [];
    [-1, 1].forEach((s) => {
      const eye = new THREE.Mesh(new THREE.SphereGeometry(0.085, 14, 14), eyeMaterial);
      eye.position.set(s * 0.17, 0.13, 0.22);
      this.head.add(eye);
      eyes.push(eye);
      const pupil = new THREE.Mesh(new THREE.SphereGeometry(0.035, 10, 10), pupilMaterial);
      pupil.position.set(s * 0.17, 0.13, 0.3);
      pupil.scale.set(0.5, 1.4, 1);
      this.head.add(pupil);
    });
    this.leftEye = eyes[0];
    this.rightEye = eyes[1];

    // Back-swept horns.
    const hornMaterial = makeMaterial(DRAGON.spike, 0.4);
    [-1, 1].forEach((s) => {
      const horn = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.3, 8), hornMaterial);
      horn.position.set(s * 0.16, 0.32, -0.04);
      horn.rotation.x = -0.5;
      horn.rotation.z = s * 0.2;
      this.head.add(horn);
    });

    // A couple of fangs under the snout.
    const toothMaterial = new THREE.MeshBasicMaterial({ color: DRAGON.tooth });
    [-0.08, 0.08].forEach((x) => {
      const tooth = new THREE.Mesh(new THREE.ConeGeometry(0.024, 0.07, 6), toothMaterial);
      tooth.position.set(x, -0.18, 0.46);
      tooth.rotation.x = Math.PI;
      this.head.add(tooth);
    });

    // Crown frill — kept as `this.antenna` for the inherited idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.28, -0.12);
    const frill = new THREE.Mesh(new THREE.ConeGeometry(0.045, 0.18, 4), makeMaterial(DRAGON.spike, 0.4));
    frill.position.y = 0.09;
    this.antenna.add(frill);
    this.head.add(this.antenna);
  }

  // Tapering segmented tail off the lower back.
  buildDragonTail() {
    this.tail = new THREE.Group();
    this.root.add(this.tail);
    this._tailSegs = [];
    const material = makeMaterial(DRAGON.body, 0.5);
    let r = 0.16;
    for (let i = 0; i < 5; i += 1) {
      const seg = new THREE.Mesh(new THREE.SphereGeometry(r, 12, 12), material);
      seg.position.set(0, -i * 0.18, -i * 0.15);
      this.tail.add(seg);
      this._tailSegs.push(seg);
      r *= 0.82;
    }
    const tip = new THREE.Mesh(new THREE.ConeGeometry(0.09, 0.22, 4), makeMaterial(DRAGON.spike, 0.4));
    tip.position.set(0, -5 * 0.18, -5 * 0.15);
    tip.rotation.x = -Math.PI / 2;
    this.tail.add(tip);
    this._tailSegs.push(tip);
  }

  // A flapping pair of wings (the reusable winged-appendage technique).
  buildWings() {
    this.wings = [];
    ...
    const ribMaterial = makeMaterial(DRAGON.body, 0.5);
    [-1, 1].forEach((sign) => {
      const wing = new THREE.Group();
      const membrane = new THREE.Mesh(new THREE.ConeGeometry(0.52, 1.1, 3), membraneMaterial);
      membrane.scale.set(1, 1, 0.08);
      membrane.rotation.z = (sign * Math.PI) / 2;
      membrane.rotation.y = sign * 0.5;
      membrane.position.set(sign * 0.58, 0.1, -0.15);
      wing.add(membrane);
      const rib = new THREE.Mesh(new THREE.CylinderGeometry(0.026, 0.015, 1.1, 6), ribMaterial);
      rib.rotation.z = (sign * Math.PI) / 2;
      rib.rotation.y = sign * 0.5;
      rib.position.set(sign * 0.58, 0.1, -0.15);
      wing.add(rib);
      this.root.add(wing);
      this.wings.push({ group: wing, sign });
    });
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyDragon tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyDragon waiting for your body pose";
  ...
  // Anchor + animate the appendages each frame: tail waves behind the body, wings flap.
  syncAttachedModel() {
    ...
    if (this.tail) {
      this.tail.position.set(tp.x, tp.y - 0.2, tp.z - 0.35);
      this.tail.rotation.y = Math.sin(t * 1.5) * 0.25;
      this._tailSegs.forEach((seg, i) => {
        seg.position.x = Math.sin(t * 2 + i * 0.7) * 0.03 * i;
      });
    }

    if (this.wings) {
      const flap = Math.sin(t * 4);
      this.wings.forEach(({ group, sign }) => {
        group.position.set(tp.x, tp.y + 0.4, tp.z - 0.2);
        group.rotation.z = sign * (0.15 + flap * 0.4);
      });
    }
  }
