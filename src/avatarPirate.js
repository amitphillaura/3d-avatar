...
// Swashbuckling pirate palette — a dark maroon coat with gold trim, weathered skin,
// a black tricorn hat, and the signature replaced limbs (steel hook + wooden peg).
const PIRATE = {
  coat: 0x5a2a3a, // dark maroon coat (bones/joints/neck/torso)
  skin: 0xf3c9a6, // weathered skin (head, normal hand)
  hat: 0x141414, // black tricorn hat
  gold: 0xffd24a, // gold belt/buttons/trim
  hook: 0xc0c6cf, // steel hook
  peg: 0x7a5a3a, // wooden peg leg
  eye: 0x2a1a14, // dark eye
  patch: 0x0a0a0a // black eyepatch
};

// Coat-maroon limbs with skin-tone arms; chunky enough to read as a heavy pirate coat.
const BONE_STYLE = {
  collar: { radius: 0.06, color: PIRATE.coat },
  hip: { radius: 0.065, color: PIRATE.coat },
  arm: { radius: 0.05, color: PIRATE.coat },
  leg: { radius: 0.06, color: PIRATE.coat },
  side: { radius: 0.05, color: PIRATE.coat }
};
...
// Brushed metal for the steel hook and gold trim/buttons.
...
export class MushyPirate {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(PIRATE.skin),
      right: this.buildHandRig(PIRATE.skin)
    };
    this.resize();
  ...
  createRig() {
    // Coat-maroon joints so the seams of the heavy pirate coat blend.
    const jointGeometry = new THREE.SphereGeometry(0.072, 20, 20);
    const jointMaterial = makeMaterial(PIRATE.coat, 0.6);
    ...
    // THE TECHNIQUE — asymmetric / replaced limb caps. Left side is a normal pirate
    // (skin hand, dark boot); right side has the swashbuckler's prosthetics: a steel
    // HOOK for the hand and a wooden PEG LEG for the foot. All four still live under the
    // exact cap keys, visible=false, added to this.root.
    this.caps = {
      leftWrist: this.buildHandCap(),
      rightWrist: this.buildHookCap(),
      leftAnkle: this.buildBootCap(),
      rightAnkle: this.buildPegCap()
    };
    ...
    // Neck connects the torso/shoulders to the head — coat-maroon collar.
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeMaterial(style.color, 0.58);
  ...
  // A plain skin hand for the unmaimed side.
  buildHandCap() {
    return new THREE.Mesh(new THREE.SphereGeometry(0.12, 18, 18), makeMaterial(PIRATE.skin, 0.55));
  }

  // The steel HOOK: a short cuff cylinder topped by a curved torus-arc hook.
  buildHookCap() {
    const hook = new THREE.Group();
    const metal = makeMetal(PIRATE.hook);

    const cuff = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.085, 0.16, 16), metal);
    cuff.position.y = 0.06;
    hook.add(cuff);

    // Curved hook: a partial torus arc opening to one side, like a J.
    const arc = new THREE.Mesh(new THREE.TorusGeometry(0.1, 0.026, 12, 24, Math.PI * 1.4), metal);
    arc.position.set(0, -0.06, 0);
    arc.rotation.z = Math.PI * 0.15;
    hook.add(arc);

    // Sharp tip so the hook reads as a point.
    const tip = new THREE.Mesh(new THREE.ConeGeometry(0.026, 0.07, 12), metal);
    tip.position.set(0.085, 0.02, 0);
  ...
  // A dark leather boot for the unmaimed leg.
  buildBootCap() {
    const boot = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.16, 0.3), makeMaterial(0x241a16, 0.7));
    return boot;
  }

  // The wooden PEG LEG: a brown cone-ish cylinder, wide at the top knee-cup, tapering
  // to a narrow tip at the bottom.
  buildPegCap() {
    const peg = new THREE.Group();
    const wood = makeMaterial(PIRATE.peg, 0.85);

    const shaft = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.03, 0.42, 14), wood);
    shaft.position.y = -0.16;
    peg.add(shaft);

    // A small cup/cap where the peg meets the knee.
    const cup = new THREE.Mesh(new THREE.SphereGeometry(0.085, 14, 14, 0, Math.PI * 2, 0, Math.PI / 2), wood);
    cup.position.y = 0.04;
    peg.add(cup);
  ...
  // Torso = maroon coat sphere with a gold belt band and a couple of gold buttons.
  buildBody() {
    this.torso = new THREE.Mesh(
      new THREE.SphereGeometry(0.82, 32, 32),
      makeMaterial(PIRATE.coat, 0.62)
    );
    this.torso.scale.set(0.92, 1.12, 0.5);
    this.root.add(this.torso);

    // Gold belt band around the waist (a thin torus hugging the torso surface).
    const belt = new THREE.Mesh(
      new THREE.TorusGeometry(0.78, 0.07, 12, 36),
      makeMetal(PIRATE.gold, 0.42)
    );
    belt.position.y = -0.5;
    belt.rotation.x = Math.PI / 2;
    this.torso.add(belt);

    // A couple of gold coat buttons down the front (+z is the front of the figure).
    const buttonMat = makeMetal(PIRATE.gold, 0.4);
    [0.34, 0.04, -0.26].forEach((y) => {
      const button = new THREE.Mesh(new THREE.SphereGeometry(0.07, 14, 14), buttonMat);
      button.position.set(0, y, 0.98);
      this.torso.add(button);
    });
  }

  // Head = skin sphere with one visible eye, an EYEPATCH over the other (disc + strap),
  // a smile, a stubbly chin, and a TRICORN HAT on top stored as this.antenna.
  buildHead() {
    this.headRadius = 0.34;
    this.head = new THREE.Mesh(
      new THREE.SphereGeometry(this.headRadius, 32, 32),
      makeMaterial(PIRATE.skin, 0.6)
    );
    this.root.add(this.head);

    const R = this.headRadius;

    // The good eye (left side from the figure's view).
    const eyeMat = new THREE.MeshBasicMaterial({ color: PIRATE.eye });
    this.leftEye = new THREE.Mesh(new THREE.SphereGeometry(0.05, 14, 14), eyeMat);
    this.leftEye.position.set(-0.12, 0.06, R * 0.88);
    this.head.add(this.leftEye);

    // The EYEPATCH over the other eye: a dark disc + a thin strap across the head.
    const patch = new THREE.Group();
    const patchMat = makeMaterial(PIRATE.patch, 0.8);
    const disc = new THREE.Mesh(new THREE.CircleGeometry(0.085, 20), patchMat);
    disc.position.set(0.12, 0.07, R * 0.92);
    patch.add(disc);

    const strapMat = makeMaterial(PIRATE.patch, 0.8);
    const strap = new THREE.Mesh(new THREE.TorusGeometry(R * 1.0, 0.018, 8, 40), strapMat);
    // Tilt the strap so it runs diagonally across the head over the patched eye.
    ...
    // A weathered smile (lower torus arc, bowed downward into a grin).
    const mouth = new THREE.Mesh(
      new THREE.TorusGeometry(0.1, 0.022, 10, 24, Math.PI),
      makeMaterial(0x7a2230, 0.55)
    );
    mouth.position.set(0, -0.16, R * 0.9);
    mouth.rotation.z = Math.PI;
    this.head.add(mouth);

    // A stubbly chin — a subtle darker patch low on the face.
    ...
    const stubble = new THREE.Mesh(new THREE.SphereGeometry(0.16, 16, 16), stubbleMat);
    stubble.position.set(0, -0.22, R * 0.62);
    stubble.scale.set(1.1, 0.7, 0.5);
    this.head.add(stubble);

    // TRICORN HAT = this.antenna (a Group, so the idle wiggle on .rotation.z sways it).
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, R * 0.82, 0);
    const hatMat = makeMaterial(PIRATE.hat, 0.7);

    // Wide flattened brim (a squashed cone reads as a tricorn brim).
    const brim = new THREE.Mesh(new THREE.ConeGeometry(0.42, 0.16, 3), hatMat);
    brim.rotation.y = Math.PI / 6;
    brim.scale.set(1, 0.5, 1);
    this.antenna.add(brim);

    // The crown of the hat.
    const crown = new THREE.Mesh(new THREE.SphereGeometry(0.2, 16, 16, 0, Math.PI * 2, 0, Math.PI / 2), hatMat);
    crown.position.y = 0.06;
    this.antenna.add(crown);

    // Gold trim around the brim edge.
    const trim = new THREE.Mesh(new THREE.TorusGeometry(0.27, 0.018, 8, 36), makeMetal(PIRATE.gold, 0.42));
    trim.rotation.x = Math.PI / 2;
    trim.position.y = 0.03;
    this.antenna.add(trim);

    // A tiny gold skull badge on the front of the hat.
    const skull = new THREE.Mesh(new THREE.SphereGeometry(0.05, 12, 12), makeMetal(PIRATE.gold, 0.4));
    skull.position.set(0, 0.1, 0.2);
    skull.scale.set(1, 1.15, 0.7);
    this.antenna.add(skull);
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyPirate tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyPirate waiting for your body pose";
  ...
  // Per-frame hook. MushyPirate's look is all static geometry, so this is a gentle no-op.
  syncAttachedModel() {}
