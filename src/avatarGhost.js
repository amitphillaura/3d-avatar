...
// Friendly floating ghost — soft translucent white, no legs (a swaying tail instead).
const GHOST = {
  body: 0xeaf2ff,
  eye: 0x10131c,
  mouth: 0x2a1430,
...
// Stubby translucent arms; the lower-body bones still exist but get hidden every frame
// (see syncAttachedModel) so the procedural tail can stand in for the legs.
const BONE_STYLE = {
  collar: { radius: 0.07, color: GHOST.body },
  hip: { radius: 0.07, color: GHOST.body },
  arm: { radius: 0.07, color: GHOST.body },
  leg: { radius: 0.06, color: GHOST.body },
  side: { radius: 0.06, color: GHOST.body }
};

// Lower-body landmarks the tail replaces.
const GHOST_LOWER = new Set([
...
// Soft translucent material for the ghost body (depthWrite off so overlaps blend).
function makeGhostMaterial(color, opacity = 0.55) {
  return new THREE.MeshStandardMaterial({
    color,
    roughness: 0.45,
    metalness: 0,
...
export class MushyGhost {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(GHOST.body),
      right: this.buildHandRig(GHOST.body)
    };
    this.resize();
  ...
  createRig() {
    ...
    const jointMaterial = makeGhostMaterial(GHOST.body, 0.55);
    ...
    // Little translucent mitt hands; ankle caps exist but stay hidden (tail covers them).
    const handMaterial = makeGhostMaterial(GHOST.body, 0.6);
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.SphereGeometry(0.13, 16, 16), handMaterial),
      rightWrist: new THREE.Mesh(new THREE.SphereGeometry(0.13, 16, 16), handMaterial),
      leftAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.12, 14, 14), handMaterial),
      rightAnkle: new THREE.Mesh(new THREE.SphereGeometry(0.12, 14, 14), handMaterial)
    };
    ...
    this.neck = new THREE.Mesh(
      new THREE.CylinderGeometry(neckStyle.radius * 0.7, neckStyle.radius, 1, 14),
      makeGhostMaterial(GHOST.body, 0.55)
    );
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeGhostMaterial(style.color, 0.55);
    ...
    // Soft translucent ghost body.
    this.torso = new THREE.Mesh(
      new THREE.SphereGeometry(0.82, 28, 28),
      makeGhostMaterial(GHOST.body, 0.5)
    );
    this.torso.scale.set(0.95, 1.1, 0.85);
  ...
  buildGhostFace() {
    const R = 0.36;
    const eyeMaterial = new THREE.MeshBasicMaterial({ color: GHOST.eye });
    const eyeGeometry = new THREE.SphereGeometry(0.075, 16, 16);
    this.leftEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.leftEye.position.set(-0.13, 0.07, R * 0.84);
    this.rightEye.position.set(0.13, 0.07, R * 0.84);
    ...
    const mouth = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: GHOST.mouth })
    );
    mouth.position.set(0, -0.12, R * 0.86);
    mouth.scale.set(1, 1.35, 0.6);
    this.head.add(mouth);

    // Rosy cheeks.
    const blushMaterial = new THREE.MeshBasicMaterial({ color: GHOST.blush, transparent: true, opacity: 0.5 });
    [-1, 1].forEach((s) => {
      const cheek = new THREE.Mesh(new THREE.SphereGeometry(0.05, 12, 12), blushMaterial);
      cheek.position.set(s * 0.2, -0.03, R * 0.78);
      cheek.scale.set(1.2, 0.8, 0.4);
      this.head.add(cheek);
    });

    // Crown wisp — kept as `this.antenna` so the inherited idle wiggle sways it.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, R * 0.85, 0);
    const wisp = new THREE.Mesh(new THREE.ConeGeometry(0.07, 0.18, 12), makeGhostMaterial(GHOST.body, 0.5));
    wisp.position.y = 0.08;
    this.antenna.add(wisp);
    this.head.add(this.antenna);
  }

  // Billowing tail that replaces the legs: a tapered skirt ending in 3 rippling lobes.
  buildTail() {
    this.tail = new THREE.Group();
    this.root.add(this.tail);
    this._tailLobes = [];
    const material = makeGhostMaterial(GHOST.body, 0.5);
    const trunk = new THREE.Mesh(
      new THREE.CylinderGeometry(0.55, 0.42, 0.7, 20, 1, true),
      material
    );
    trunk.position.y = -0.35;
    this.tail.add(trunk);
    [-0.32, 0, 0.32].forEach((x) => {
      const lobe = new THREE.Mesh(new THREE.SphereGeometry(0.17, 14, 14), material);
      lobe.position.set(x, -0.7, 0);
      lobe.scale.set(1, 1.3, 1);
      this.tail.add(lobe);
      this._tailLobes.push(lobe);
    });
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyGhost tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyGhost waiting for your body pose";
  ...
  // Per-frame extras (called by the base animate loop): hide the lower body so the tail
  // stands in for the legs, then anchor + ripple the tail under the torso.
  syncAttachedModel() {
    this.bones.forEach(({ from, to, mesh }) => {
      if (GHOST_LOWER.has(from) || GHOST_LOWER.has(to)) mesh.visible = false;
    });
    this.joints.forEach((joint, name) => {
      if (GHOST_LOWER.has(name)) joint.visible = false;
    });
    if (this.caps.leftAnkle) this.caps.leftAnkle.visible = false;
    if (this.caps.rightAnkle) this.caps.rightAnkle.visible = false;

    if (!this.tail || !this.torso) return;
    const t = (performance.now() - this.startedAt) / 1000;
    this.tail.position.set(
      this.torso.position.x + Math.sin(t * 1.6) * 0.05,
      this.torso.position.y - 0.55,
      this.torso.position.z
    );
    this.tail.rotation.z = Math.sin(t * 1.4) * 0.12;
    this._tailLobes.forEach((lobe, i) => {
      lobe.position.y = -0.7 + Math.sin(t * 3 + i * 1.1) * 0.06;
    });
  }
