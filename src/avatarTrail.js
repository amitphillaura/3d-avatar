...
// Glowing light-dancer — cyan body, magenta + cyan ribbon trails off the hands.
const TRAIL = { body: 0x39e6ff, trailL: 0xff3df0, trailR: 0x39e6ff, eye: 0xffffff };

const BONE_STYLE = {
  collar: { radius: 0.055, color: TRAIL.body },
  hip: { radius: 0.06, color: TRAIL.body },
  arm: { radius: 0.045, color: TRAIL.body },
  leg: { radius: 0.052, color: TRAIL.body },
  side: { radius: 0.04, color: TRAIL.body }
};
...
// Self-lit glowing material for the light-dancer body.
...
export class MushyTrail {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(TRAIL.body),
      right: this.buildHandRig(TRAIL.body)
    };
    this.resize();
  ...
  createRig() {
    ...
    const jointMaterial = makeGlow(TRAIL.body);
    ...
    const capMaterial = makeGlow(TRAIL.body);
    this.caps = {
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeGlow(style.color);
    ...
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 32, 32), makeGlow(TRAIL.body));
    this.torso.scale.set(0.85, 1.15, 0.36);
    this.root.add(this.torso);

    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.32, 32, 32), makeGlow(TRAIL.body));
    this.root.add(this.head);

    // Bright additive eyes.
    ...
    // Glowing antenna spike.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.33, 0);
    const antennaSpike = new THREE.Mesh(new THREE.ConeGeometry(0.03, 0.4, 8), makeGlow(TRAIL.body));
    antennaSpike.position.y = 0.2;
    this.antenna.add(antennaSpike);
    this.head.add(this.antenna);

    // THE TECHNIQUE — history-buffer ribbon trails streaming off each wrist.
    const N = 26;
    this._trails = [
      { key: "leftWrist", color: TRAIL.trailL },
      { key: "rightWrist", color: TRAIL.trailR }
    ].map(({ key, color }) => {
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyTrail tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyTrail waiting for your body pose";
  ...
  // THE TECHNIQUE — push each wrist's live position into a history ring and rewrite the
  // line geometry so a glowing ribbon streaks from where the hand was to where it is.
  syncAttachedModel(delta) {
    if (!this._trails) return;
    this._trails.forEach((trail) => {
      const p = this.points.get(trail.key);
      if (!p) return;
      const history = trail.history;
      // Shift oldest out, push the live position to the front.
