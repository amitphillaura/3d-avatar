...
// Blocky Minecraft/Roblox build — chunky voxel limbs with a pixel-art face.
const VOXEL = {
  skin: 0xd9a06b,
  shirt: 0x3aa0ff,
  pants: 0x394a6b,
...
const BONE_STYLE = {
  collar: { radius: 0.1, color: VOXEL.shirt },
  hip: { radius: 0.11, color: VOXEL.pants },
  arm: { radius: 0.09, color: VOXEL.skin },
  leg: { radius: 0.11, color: VOXEL.pants },
  side: { radius: 0.1, color: VOXEL.shirt }
};
...
// Matte, flat-shaded blocky material — no gloss, reads like a Minecraft cube.
...
// Pixel-art face baked into a 16x16 canvas, sampled nearest-neighbor so it stays crisp.
function makePixelFaceTexture(skinColor) {
  const c = document.createElement("canvas");
  c.width = c.height = 16;
  const ctx = c.getContext("2d");

  const hex = `#${skinColor.toString(16).padStart(6, "0")}`;
  ctx.fillStyle = hex;
  ctx.fillRect(0, 0, 16, 16);

  // Hair row across the top.
  ctx.fillStyle = "#4a3322";
  ctx.fillRect(0, 0, 16, 3);
  ctx.fillRect(0, 3, 2, 2);
  ctx.fillRect(14, 3, 2, 2);

  // Two dark eye squares.
  ctx.fillStyle = "#1a1410";
  ctx.fillRect(4, 7, 2, 2);
  ctx.fillRect(10, 7, 2, 2);

  // White glints in the eyes.
...
export class MushyVoxel {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(VOXEL.skin),
      right: this.buildHandRig(VOXEL.skin)
    };
    this.resize();
  ...
  createRig() {
    // Blocky joints — small cubes instead of spheres.
    const jointGeometry = new THREE.BoxGeometry(0.12, 0.12, 0.12);
    const jointMaterial = makeVoxelMaterial(VOXEL.skin);
    ...
    // Blocky end-caps: cube fists for wrists, slab boots for ankles.
    const fistGeometry = new THREE.BoxGeometry(0.26, 0.26, 0.26);
    const footGeometry = new THREE.BoxGeometry(0.28, 0.18, 0.4);
    const fistMaterial = makeVoxelMaterial(VOXEL.skin);
    const shoeMaterial = makeVoxelMaterial(VOXEL.shoe);
    this.caps = {
      leftWrist: new THREE.Mesh(fistGeometry, fistMaterial),
      rightWrist: new THREE.Mesh(fistGeometry, fistMaterial),
      leftAnkle: new THREE.Mesh(footGeometry, shoeMaterial),
      rightAnkle: new THREE.Mesh(footGeometry, shoeMaterial)
    };
    ...
    this.neck = new THREE.Mesh(
      ...
      makeVoxelMaterial(VOXEL.skin)
    );
    this.neck.visible = false;
    this.root.add(this.neck);

    // Chunky box limbs — setCylinderBetween stretches the box along Y between joints.
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeVoxelMaterial(style.color);
    ...
    // Box torso (shirt) — scaled like the original torso.
    this.torso = new THREE.Mesh(
      new THREE.BoxGeometry(1, 1, 1),
      makeVoxelMaterial(VOXEL.shirt)
    );
    this.torso.scale.set(0.95, 1.1, 0.55);
    this.root.add(this.torso);

    // Box head (skin cube) with a pixel-art face plane on its +z face.
    const headSize = 0.6;
    ...
    this.buildPixelFace(headSize);

    // Tiny invisible eye boxes to satisfy the contract (real eyes are drawn in the face).
    ...
    // Blocky hair tuft on top of the head, kept as `this.antenna` for the idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, headSize * 0.5, 0);
    const hairMaterial = makeVoxelMaterial(VOXEL.hair);
    const tuftA = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.12, 0.5), hairMaterial);
    tuftA.position.y = 0.06;
    const tuftB = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.18, 0.18), hairMaterial);
    tuftB.position.y = 0.18;
    this.antenna.add(tuftA, tuftB);
    this.head.add(this.antenna);
  }

  // THE TECHNIQUE — a 16x16 pixel-art face on a thin plane in front of the head cube.
  buildPixelFace(headSize) {
    const tex = makePixelFaceTexture(VOXEL.skin);
    const faceMaterial = new THREE.MeshBasicMaterial({ map: tex, transparent: true });
    this.pixelFace = new THREE.Mesh(new THREE.PlaneGeometry(0.5, 0.5), faceMaterial);
    this.pixelFace.position.set(0, 0, headSize * 0.5 + 0.01);
    this.head.add(this.pixelFace);
  }
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyVoxel tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyVoxel waiting for your body pose";
  ...
  // Per-frame work for the blocky build. The pixel face is static, so this stays a no-op.
