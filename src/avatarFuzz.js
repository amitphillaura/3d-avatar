...
// MushyFuzz — a round fuzzy purple monster covered in dense fur tufts.
const FUZZ = { fur: 0x9b6bff, furTip: 0xc9a8ff, belly: 0xe7d8ff, eye: 0x1a1018, mouth: 0x3a1f2a };
const FUR = FUZZ.fur;

const BONE_STYLE = {
  collar: { radius: 0.08, color: FUZZ.fur },
  hip: { radius: 0.09, color: FUZZ.fur },
  arm: { radius: 0.07, color: FUZZ.fur },
  leg: { radius: 0.085, color: FUZZ.fur },
  side: { radius: 0.075, color: FUZZ.fur }
};
...
export class MushyFuzz {
  constructor(mount, metaElement, options = {}) {
    ...
    this.scene.background = new THREE.Color(0x120e1c);
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(FUR),
      right: this.buildHandRig(FUR)
    };
    this.resize();
  ...
  createRig() {
    const jointGeometry = new THREE.SphereGeometry(0.085, 20, 20);
    const jointMaterial = makeMaterial(FUR, 0.7);
    ...
    // Chunky fuzzy paws and feet.
    const handGeometry = new THREE.SphereGeometry(0.14, 18, 18);
    const footGeometry = new THREE.SphereGeometry(0.15, 18, 18);
    const furCapMaterial = makeMaterial(FUR, 0.7);
    this.caps = {
      leftWrist: new THREE.Mesh(handGeometry, furCapMaterial),
      rightWrist: new THREE.Mesh(handGeometry, furCapMaterial),
      leftAnkle: new THREE.Mesh(footGeometry, furCapMaterial),
      rightAnkle: new THREE.Mesh(footGeometry, furCapMaterial)
    };
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeMaterial(style.color, 0.7);
    ...
    // Round fuzzy purple body.
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 32, 32), makeMaterial(FUR, 0.7));
    this.torso.scale.set(0.95, 1.05, 0.85);
    this.root.add(this.torso);

    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.4, 32, 32), makeMaterial(FUR, 0.7));
    this.root.add(this.head);

    // THE TECHNIQUE — dense fur-tuft surface scatter via a fibonacci sphere.
    const tuftGeo = new THREE.ConeGeometry(0.05, 0.18, 5);
    const placeFur = (host, R, count, color) => {
      const mat = makeMaterial(color, 0.7);
      for (let i = 0; i < count; i++) {
        const y = 1 - (i / (count - 1)) * 2;
        const r = Math.sqrt(Math.max(0, 1 - y * y));
        const theta = i * 2.399963;
        const dir = new THREE.Vector3(Math.cos(theta) * r, y, Math.sin(theta) * r);
        // Skip tufts on the front-lower head where the face sits.
        if (host === this.head && dir.z > 0.35 && dir.y < 0.2) continue;
        const tuft = new THREE.Mesh(tuftGeo, mat);
        tuft.position.copy(dir).multiplyScalar(R);
        tuft.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), dir.clone().normalize());
        host.add(tuft);
      }
    };
    placeFur(this.torso, 0.82, 70, FUR);
    placeFur(this.head, 0.4, 36, FUR);

    // Big cute eyes pushed out so they sit on top of the fur.
    const scleraMaterial = makeMaterial(FUZZ.belly, 0.4);
    const eyeMaterial = new THREE.MeshBasicMaterial({ color: FUZZ.eye });
    const eyeR = 0.4;
    [-1, 1].forEach((s) => {
      const sclera = new THREE.Mesh(new THREE.SphereGeometry(0.085, 16, 16), scleraMaterial);
      sclera.position.set(s * 0.14, 0.06, eyeR * 1.02);
      const pupil = new THREE.Mesh(new THREE.SphereGeometry(0.045, 12, 12), eyeMaterial);
      pupil.position.set(s * 0.14, 0.06, eyeR * 1.08);
      this.head.add(sclera, pupil);
      if (s < 0) this.leftEye = pupil;
      else this.rightEye = pupil;
    });

    // A friendly smile.
    const smile = new THREE.Mesh(
      new THREE.TorusGeometry(0.1, 0.022, 8, 18, Math.PI),
      new THREE.MeshBasicMaterial({ color: FUZZ.mouth })
    );
    smile.position.set(0, -0.1, eyeR * 1.0);
    smile.rotation.z = Math.PI;
    this.head.add(smile);

    // Antenna = one bigger fur tuft on the crown for the idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.4, 0);
    const crownTuft = new THREE.Mesh(
      new THREE.ConeGeometry(0.09, 0.34, 6),
      makeMaterial(FUZZ.furTip, 0.7)
    );
    crownTuft.position.y = 0.14;
    this.antenna.add(crownTuft);
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyFuzz tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyFuzz waiting for your body pose";
  ...
  // Gentle breathing sway — ripple the body fur tufts so the monster looks alive.
  syncAttachedModel() {
    if (!this.torso) return;
    const t = (performance.now() - this.startedAt) / 1000;
    const children = this.torso.children;
    for (let i = 0; i < children.length; i++) {
      children[i].rotation.z = Math.sin(t * 2 + i * 0.5) * 0.08;
    }
  }
