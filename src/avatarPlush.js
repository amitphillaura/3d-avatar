...
// Teddy-bear plushie palette — matte brown fabric with cream inner accents,
// dark stitch/button details and pink cheeks.
const PLUSH = {
  fur: 0xb87b4a, // brown body fabric
  inner: 0xe8c9a0, // muzzle / ear inner / paw pads
  stitch: 0x5a3a22, // sewn seam marks
  eye: 0x2a1a10, // button eyes
  nose: 0x2a1a10, // stitched nose
  cheek: 0xff9ec4 // pink blush
};

// Chunky matte-fabric limbs — same brown fur all over so the plush reads as one
// stuffed body. High roughness (set per-mesh via makeMaterial) keeps it cloth-like.
const BONE_STYLE = {
  collar: { radius: 0.08, color: PLUSH.fur },
  hip: { radius: 0.09, color: PLUSH.fur },
  arm: { radius: 0.07, color: PLUSH.fur },
  leg: { radius: 0.085, color: PLUSH.fur },
  side: { radius: 0.075, color: PLUSH.fur }
};
...
// Matte fabric: very high roughness, zero metalness, no self-glow — reads as soft
// stuffed cloth rather than the slightly shiny default standard material.
function makeFabric(color, roughness = 0.95) {
  return new THREE.MeshStandardMaterial({
    color,
    roughness,
    metalness: 0,
...
export class MushyPlush {
  constructor(mount, metaElement, options = {}) {
    ...
    this.scene.background = new THREE.Color(0x140f0a);
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(PLUSH.fur),
      right: this.buildHandRig(PLUSH.fur)
    };
    this.resize();
  ...
  createRig() {
    // Soft brown fabric joints so the stuffed limbs blend at the seams.
    const jointGeometry = new THREE.SphereGeometry(0.085, 18, 18);
    const jointMaterial = makeFabric(PLUSH.fur);
    ...
    // Round brown paw caps (with a small cream pad) for hands, chunky pads for feet.
    const pawMaterial = makeFabric(PLUSH.fur);
    const padMaterial = makeFabric(PLUSH.inner);
    const makePaw = (radius) => {
      const paw = new THREE.Group();
      const ball = new THREE.Mesh(new THREE.SphereGeometry(radius, 16, 16), pawMaterial);
      paw.add(ball);
      const pad = new THREE.Mesh(new THREE.SphereGeometry(radius * 0.5, 12, 12), padMaterial);
      pad.position.set(0, 0, radius * 0.78);
      pad.scale.set(1, 1, 0.4);
      paw.add(pad);
      return paw;
    };
    this.caps = {
      leftWrist: makePaw(0.14),
      rightWrist: makePaw(0.14),
      leftAnkle: makePaw(0.15),
      rightAnkle: makePaw(0.15)
    };
    ...
    // Neck connects the torso/shoulders to the head — brown fabric.
    const neckStyle = BONE_STYLE.collar;
    this.neck = new THREE.Mesh(
      new THREE.CylinderGeometry(neckStyle.radius * 0.85, neckStyle.radius, 1, 16),
      makeFabric(PLUSH.fur)
    );
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const geometry = new THREE.CylinderGeometry(style.radius, style.radius, 1, 16);
      const material = makeFabric(style.color);
    ...
    // Round chunky stuffed belly.
    this.torso = new THREE.Mesh(
      new THREE.SphereGeometry(0.82, 32, 32),
      makeFabric(PLUSH.fur)
    );
    this.torso.scale.set(1.0, 1.08, 0.7);
    this.root.add(this.torso);

    // A row of stitch seams down the torso center front (children of the torso so
    // they follow its position/scale) — the sewn-plush technique.
    this.buildStitchSeam();
  ...
  // A vertical row of tiny dark stitch marks down the front of the torso, evoking a
  // sewn seam. Parented to `this.torso` so they ride along with the belly each frame.
  buildStitchSeam() {
    const stitchMat = makeFabric(PLUSH.stitch, 0.85);
    const stitchGeo = new THREE.BoxGeometry(0.03, 0.05, 0.02);
    const count = 8;
    for (let i = 0; i < count; i += 1) {
      const stitch = new THREE.Mesh(stitchGeo, stitchMat);
      // Spread up the belly front. Torso is a unit-radius sphere scaled later, so use
      // local sphere coordinates (radius ~0.82, front face ~+z).
      const y = 0.5 - (i / (count - 1)) * 1.0; // top to bottom
      stitch.position.set(0, y * 0.7, 0.8);
      this.torso.add(stitch);
    }
  }

  // Procedural teddy-bear face: round ears with cream inner discs, a cream muzzle
  // with a dark nose and a stitched smile, dark button eyes, pink cheeks, and a small
  // bow as `this.antenna`. Everything parents to `this.head` so it follows the head.
  buildFace() {
    const R = this.headRadius;
    const furMat = makeFabric(PLUSH.fur);
    const innerMat = makeFabric(PLUSH.inner);

    // Round ears on top, each with a smaller cream inner disc facing forward.
    const buildEar = (sign) => {
      const ear = new THREE.Group();
      ear.position.set(sign * 0.28, R * 0.82, 0.02);
      const outer = new THREE.Mesh(new THREE.SphereGeometry(0.13, 16, 16), furMat);
      ear.add(outer);
      const innerDisc = new THREE.Mesh(new THREE.SphereGeometry(0.075, 14, 14), innerMat);
      innerDisc.position.set(0, 0, 0.07);
      innerDisc.scale.set(1, 1, 0.35);
      ear.add(innerDisc);
      this.head.add(ear);
      return ear;
    };
    ...
    // Cream muzzle — a flattened sphere on the lower-front of the face.
    this.muzzle = new THREE.Mesh(new THREE.SphereGeometry(0.17, 18, 18), innerMat);
    this.muzzle.position.set(0, -0.1, R * 0.86);
    this.muzzle.scale.set(1.1, 0.85, 0.7);
    this.head.add(this.muzzle);

    // Dark stitched nose at the top of the muzzle.
    this.nose = new THREE.Mesh(new THREE.SphereGeometry(0.05, 14, 14), makeFabric(PLUSH.nose, 0.7));
    this.nose.position.set(0, -0.02, R * 0.99);
    this.nose.scale.set(1.2, 0.85, 0.8);
    this.head.add(this.nose);

    // Stitched smile under the nose — bottom half of a thin torus bowing down (∪).
    this.mouth = new THREE.Mesh(
      new THREE.TorusGeometry(0.07, 0.012, 8, 22, Math.PI),
      makeFabric(PLUSH.stitch, 0.7)
    );
    this.mouth.position.set(0, -0.12, R * 0.96);
    this.mouth.rotation.z = Math.PI;
    this.head.add(this.mouth);

    // Button eyes — short dark cylinders facing forward (flat disc faces the camera).
    const eyeMat = makeFabric(PLUSH.eye, 0.5);
    const buildEye = (sign) => {
      const eye = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 0.03, 16), eyeMat);
      eye.rotation.x = Math.PI / 2; // flat face toward +z
      eye.position.set(sign * 0.16, 0.08, R * 0.93);
      this.head.add(eye);
      return eye;
    };
    this.leftEye = buildEye(-1);
    this.rightEye = buildEye(1);
    ...
    const buildCheek = (sign) => {
      const cheek = new THREE.Mesh(new THREE.SphereGeometry(0.07, 14, 14), cheekMat);
      cheek.position.set(sign * 0.27, -0.05, R * 0.78);
      cheek.scale.set(1.1, 0.8, 0.4);
      this.head.add(cheek);
      return cheek;
    };
    this.leftCheek = buildCheek(-1);
    this.rightCheek = buildCheek(1);

    // A little bow on top as `this.antenna` so the idle wiggle (antenna.rotation.z)
    // sways it. A Group has .rotation just like a Mesh, so the base code works.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0.18, R * 0.95, 0.04);
    const bowMat = makeFabric(PLUSH.cheek, 0.85);
    const knot = new THREE.Mesh(new THREE.SphereGeometry(0.04, 12, 12), bowMat);
    this.antenna.add(knot);
    const buildLoop = (sign) => {
      const loop = new THREE.Mesh(new THREE.SphereGeometry(0.055, 12, 12), bowMat);
      loop.position.set(sign * 0.07, 0, 0);
      loop.scale.set(1, 0.7, 0.5);
      this.antenna.add(loop);
    };
    buildLoop(-1);
    buildLoop(1);
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyPlush tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyPlush waiting for your body pose";
  ...
  // The plush toy is mostly static (the base idle-bobs the whole rig). A gentle ear
  // wiggle adds a touch of soft-toy life without fighting the base transforms.
  syncAttachedModel(delta) {
    if (!this.leftEar || !this.rightEar) return;
    const t = (performance.now() - this.startedAt) / 1000;
    const wiggle = Math.sin(t * 2.2) * 0.08;
    this.leftEar.rotation.z = wiggle;
    this.rightEar.rotation.z = -wiggle;
  }
