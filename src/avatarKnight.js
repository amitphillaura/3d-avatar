...
// Plate-armor knight — brushed steel, dark gunmetal joints, gold trim, a red plume,
// and faint glowing eyes peeking through the visor slit.
const KNIGHT = {
  steel: 0xb8c0cc, // armor plate / arms
  dark: 0x3a4150, // ball joints / hip + leg plating / panel gaps
  gold: 0xffd24a, // trim band, shield boss + cross emblem
  plume: 0xc0392b, // red helm plume feathers
  eyeGlow: 0x9fe8ff // glowing eyes behind the visor
};

// Thick steel struts for the arms; dark gunmetal for the lower body and shoulder lines.
const BONE_STYLE = {
  collar: { radius: 0.085, color: KNIGHT.dark },
  hip: { radius: 0.1, color: KNIGHT.dark },
  arm: { radius: 0.075, color: KNIGHT.steel },
  leg: { radius: 0.095, color: KNIGHT.dark },
  side: { radius: 0.08, color: KNIGHT.dark }
};
...
// Brushed-metal material for armor plate (reads as steel under the scene lighting).
...
export class MushyKnight {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(KNIGHT.steel),
      right: this.buildHandRig(KNIGHT.steel)
    };
    this.resize();
  ...
  createRig() {
    // Steel ball joints rivet the armor plates together.
    const jointGeometry = new THREE.SphereGeometry(0.085, 16, 16);
    const jointMaterial = makeMetal(KNIGHT.steel, 0.42);
    ...
    // Gauntlet fists for the wrists, flattened steel boots for the ankles.
    const handMaterial = makeMetal(KNIGHT.steel, 0.36);
    const footMaterial = makeMetal(KNIGHT.steel, 0.42);
    this.caps = {
      leftWrist: new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.24, 0.24), handMaterial),
      rightWrist: new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.24, 0.24), handMaterial),
      leftAnkle: new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.16, 0.42), footMaterial),
      rightAnkle: new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.16, 0.42), footMaterial)
    };
    ...
    // Dark gorget connecting the breastplate to the helm.
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeMetal(style.color, 0.42);
    ...
    // Steel breastplate with a gold trim band and shoulder pauldrons.
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 26, 26), makeMetal(KNIGHT.steel, 0.4));
    this.torso.scale.set(0.9, 1.12, 0.5);
    this.root.add(this.torso);

    const trim = new THREE.Mesh(
      new THREE.TorusGeometry(0.78, 0.05, 10, 36),
      makeMetal(KNIGHT.gold, 0.35)
    );
    trim.rotation.x = Math.PI / 2;
    trim.position.y = -0.2;
    this.torso.add(trim);

    const pauldronGeometry = new THREE.SphereGeometry(0.34, 18, 18);
    const pauldronMaterial = makeMetal(KNIGHT.steel, 0.38);
    ...
    leftPauldron.scale.set(1, 0.5, 1);
    rightPauldron.scale.set(1, 0.5, 1);
    leftPauldron.position.set(-0.72, 0.62, 0);
    rightPauldron.position.set(0.72, 0.62, 0);
    this.torso.add(leftPauldron, rightPauldron);

    // Helm: a steel sphere with a dark visor slit, glowing eyes, and a red plume.
    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.33, 26, 26), makeMetal(KNIGHT.steel, 0.36));
    this.root.add(this.head);
    this.buildHelm();

    // THE TECHNIQUE — a shield that rides the left forearm (anchored to leftWrist each frame).
    this.buildShield();
  }

  // Helm features: a dark visor slit, two glowing eyes peeking through, and a red plume
  // group used as `this.antenna` so the inherited idle wiggle sways it.
  buildHelm() {
    const visor = new THREE.Mesh(
      new THREE.BoxGeometry(0.4, 0.07, 0.06),
      makeMetal(KNIGHT.dark, 0.5)
    );
    visor.position.set(0, 0.04, 0.31);
    this.head.add(visor);

    const eyeMaterial = new THREE.MeshBasicMaterial({ color: KNIGHT.eyeGlow });
    this.leftEye = new THREE.Mesh(new THREE.SphereGeometry(0.035, 12, 12), eyeMaterial);
    this.rightEye = new THREE.Mesh(new THREE.SphereGeometry(0.035, 12, 12), eyeMaterial);
    this.leftEye.position.set(-0.1, 0.04, 0.33);
    this.rightEye.position.set(0.1, 0.04, 0.33);
    this.head.add(this.leftEye, this.rightEye);

    // Red plume — a couple of flattened cone feathers on a swayable group.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.3, -0.04);
    const featherMaterial = makeMaterial(KNIGHT.plume, 0.5);
    [-0.05, 0.05].forEach((offsetX, index) => {
      const feather = new THREE.Mesh(new THREE.ConeGeometry(0.07, 0.34, 10), featherMaterial);
      feather.scale.set(1, 1, 0.45);
      feather.position.set(offsetX, 0.18, -0.02);
      feather.rotation.z = (index === 0 ? 1 : -1) * 0.12;
      this.antenna.add(feather);
    });
    this.head.add(this.antenna);
  }

  // Rounded shield with a gold central boss and a gold cross emblem. Parented to `this.root`
  // (NOT the body) so `syncAttachedModel` can ride it on the left wrist independently.
  buildShield() {
    this._shield = new THREE.Group();

    const board = new THREE.Mesh(
      new THREE.CylinderGeometry(0.3, 0.3, 0.06, 24),
      makeMetal(KNIGHT.steel, 0.4)
    );
    board.rotation.x = Math.PI / 2;
    this._shield.add(board);

    const boss = new THREE.Mesh(new THREE.SphereGeometry(0.07, 14, 14), makeMetal(KNIGHT.gold, 0.32));
    boss.position.z = 0.06;
    this._shield.add(boss);

    const crossMaterial = makeMetal(KNIGHT.gold, 0.34);
    const crossV = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.42, 0.02), crossMaterial);
    const crossH = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.05, 0.02), crossMaterial);
    crossV.position.z = 0.05;
    crossH.position.z = 0.05;
    this._shield.add(crossV, crossH);
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyKnight tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyKnight waiting for your body pose";
