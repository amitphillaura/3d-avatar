...
// Faceted crystal golem — amethyst shell with a glowing inner core (faked refraction).
const GEM = { gem: 0x8a7bff, gem2: 0x6be0ff, core: 0xfff0ff };

const BONE_STYLE = {
  collar: { radius: 0.06, color: GEM.gem2 },
  hip: { radius: 0.065, color: GEM.gem },
  arm: { radius: 0.05, color: GEM.gem },
  leg: { radius: 0.058, color: GEM.gem },
  side: { radius: 0.045, color: GEM.gem2 }
};
...
// Translucent flat-shaded crystal shell.
...
// Solid emissive inner core seen through the shell.
...
export class MushyGem {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(GEM.gem),
      right: this.buildHandRig(GEM.gem)
    };
    this.resize();
  ...
  createRig() {
    // Low-poly faceted gem joints.
    const jointGeometry = new THREE.OctahedronGeometry(0.085, 0);
    const jointMaterial = makeGem(GEM.gem2);
    ...
    // Faceted crystal end-caps so wrists read as hands and ankles as feet.
    const handGeometry = new THREE.OctahedronGeometry(0.13, 0);
    const footGeometry = new THREE.OctahedronGeometry(0.14, 0);
    const capMaterial = makeGem(GEM.gem);
    this.caps = {
    ...
    this.neck = new THREE.Mesh(
      new THREE.CylinderGeometry(neckStyle.radius * 0.7, neckStyle.radius * 0.9, 1, 5),
      makeGem(GEM.gem2)
    );
    this.neck.visible = false;
    this.root.add(this.neck);

    // Faceted bones — pentagonal cylinders for a crystalline look.
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeGem(style.color);
    ...
    // Translucent low-poly crystal torso with a glowing inner core.
    this.torso = new THREE.Mesh(new THREE.IcosahedronGeometry(0.82, 0), makeGem(GEM.gem));
    this.torso.scale.set(0.9, 1.12, 0.6);
    this.root.add(this.torso);
    const torsoCore = new THREE.Mesh(new THREE.IcosahedronGeometry(0.32, 0), makeCore(GEM.core));
    this.torso.add(torsoCore);

    // Faceted crystal head with its own inner core.
    this.head = new THREE.Mesh(new THREE.IcosahedronGeometry(0.34, 0), makeGem(GEM.gem));
    this.root.add(this.head);
    const headCore = new THREE.Mesh(new THREE.IcosahedronGeometry(0.14, 0), makeCore(GEM.core));
    this.head.add(headCore);

    // Inner cores pulse over time (see syncAttachedModel).
    this._cores = [torsoCore, headCore];

    // Tiny glowing eye gems on the head front.
    const eyeGeometry = new THREE.OctahedronGeometry(0.05, 0);
    const eyeMaterial = makeCore(GEM.gem2);
    this.leftEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    this.head.add(this.leftEye, this.rightEye);
    this.leftEye.position.set(-0.11, 0.06, 0.3);
    this.rightEye.position.set(0.11, 0.06, 0.3);

    // Tall crystal shard on top, kept as `this.antenna` for the inherited idle wiggle.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.33, 0);
    const shard = new THREE.Mesh(new THREE.OctahedronGeometry(0.1, 0), makeGem(GEM.gem2));
    shard.scale.set(0.4, 2.6, 0.4);
    shard.position.y = 0.16;
    this.antenna.add(shard);
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyGem tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyGem waiting for your body pose";
  ...
  // THE TECHNIQUE — fake refraction: pulse the emissive inner cores so the glow
  // visible through the translucent crystal shell breathes over time.
  syncAttachedModel(delta) {
    if (!this._cores) return;
