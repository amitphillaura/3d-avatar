...
// MushyAqua — a clear water elemental: a translucent blue shell with a rippling inner
// core and a tiny fish + bubble suspended inside.
const AQUA = { water: 0x3fb6ff, deep: 0x1f7fd6, bubble: 0xeaffff, fish: 0x1a2a3a, eye: 0x10242c };

const BONE_STYLE = {
  collar: { radius: 0.055, color: AQUA.water },
  hip: { radius: 0.06, color: AQUA.water },
  arm: { radius: 0.045, color: AQUA.water },
  leg: { radius: 0.052, color: AQUA.water },
  side: { radius: 0.04, color: AQUA.water }
};
...
// Clear, glassy water — translucent with a faint self-glow, no depth-write so the
// inner core / fish / bubble read through the shell.
function makeWater(color, opacity = 0.45) {
  return new THREE.MeshStandardMaterial({
    color,
    transparent: true,
    opacity,
...
export class MushyAqua {
  constructor(mount, metaElement, options = {}) {
    ...
    this.createRig();
    this.hands = {
      left: this.buildHandRig(AQUA.water),
      right: this.buildHandRig(AQUA.water)
    };
    this.resize();
  ...
  createRig() {
    ...
    const jointMaterial = makeWater(AQUA.water);
    ...
    const capMaterial = makeWater(AQUA.water);
    this.caps = {
    ...
    BONES.forEach(([from, to, variant]) => {
      ...
      const material = makeWater(style.color);
    ...
    // The clear water shell.
    this.torso = new THREE.Mesh(new THREE.SphereGeometry(0.82, 32, 32), makeWater(AQUA.water, 0.4));
    this.torso.scale.set(0.85, 1.15, 0.36);
    this.root.add(this.torso);

    // THE TECHNIQUE — children of the shell that we animate every frame:
    //   inner ripple core + a tiny fish + a rising bubble suspended inside.
    this._waterCore = new THREE.Mesh(new THREE.SphereGeometry(0.62, 24, 24), makeWater(AQUA.deep, 0.4));
    this.torso.add(this._waterCore);

    this._bubble = new THREE.Mesh(new THREE.SphereGeometry(0.09, 14, 14), makeWater(AQUA.bubble, 0.5));
    this._bubble.position.set(0.18, -0.3, 0.18);
    this.torso.add(this._bubble);

    this._fish = new THREE.Group();
    const fishMaterial = makeMaterial(AQUA.fish, 0.4);
    const fishBody = new THREE.Mesh(new THREE.SphereGeometry(0.11, 14, 14), fishMaterial);
    fishBody.scale.set(1.4, 0.7, 0.5);
    this._fish.add(fishBody);
    const fishTail = new THREE.Mesh(new THREE.ConeGeometry(0.07, 0.13, 10), fishMaterial);
    fishTail.rotation.z = Math.PI / 2;
    fishTail.position.set(-0.18, 0, 0);
    this._fish.add(fishTail);
    this.torso.add(this._fish);

    // Head — a clear water sphere with simple dark eyes + a smile on the front.
    this.head = new THREE.Mesh(new THREE.SphereGeometry(0.32, 32, 32), makeWater(AQUA.water, 0.4));
    this.root.add(this.head);

    const eyeMaterial = new THREE.MeshBasicMaterial({ color: AQUA.eye });
    ...
    const smile = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: AQUA.eye })
    );
    smile.position.set(0, -0.05, 0.3);
    smile.rotation.z = Math.PI;
    this.head.add(smile);

    // Antenna — a little water droplet (teardrop = sphere + small cone) on top.
    this.antenna = new THREE.Group();
    this.antenna.position.set(0, 0.33, 0);
    const dropletMaterial = makeWater(AQUA.water, 0.5);
    const dropletBulb = new THREE.Mesh(new THREE.SphereGeometry(0.07, 14, 14), dropletMaterial);
    this.antenna.add(dropletBulb);
    const dropletTip = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.11, 12), dropletMaterial);
    dropletTip.position.set(0, 0.09, 0);
    this.antenna.add(dropletTip);
  ...
  refreshMeta() {
    ...
    this.metaElement.textContent = `MushyAqua tracking · ${parts.join(" · ")}`;
  }
  ...
  animateIdle(delta) {
    ...
    if (!bodyTracking && !faceTracking && !handTracking) {
      ...
      this.activePoints.clear();
      this.metaElement.textContent = "MushyAqua waiting for your body pose";
  ...
  // THE TECHNIQUE — animate only the children we added to this.torso. The base sets
  // this.torso.scale / this.head.scale every frame, so we never touch those here.
  syncAttachedModel(delta) {
    if (!this._fish) return;
    const t = (performance.now() - this.startedAt) / 1000;

    // Ripple the inner core only.
    if (this._waterCore) {
      this._waterCore.scale.set(
        1 + Math.sin(t * 3) * 0.06,
        1 + Math.sin(t * 3.7 + 1) * 0.06,
        1 + Math.sin(t * 2.5 + 2) * 0.06
      );
    }

    // Bob the bubble slowly upward, then reset to the bottom.
    if (this._bubble) {
      this._bubble.position.y = ((t * 0.3) % 1) * 0.6 - 0.3;
    }

    // Swim the fish in a slow orbit, facing along its path.
    this._fish.position.set(Math.cos(t) * 0.3, Math.sin(t * 0.7) * 0.2, Math.sin(t) * 0.3);
    this._fish.rotation.y = t;
  }
